...
const FullscreenManager = {
  ...
  enter: function () {
    ...
    app.classList.add('fullscreen-mode', 'pregnant-module-active');
  ...
  exit: function () {
    ...
    app.classList.remove('fullscreen-mode', 'pregnant-module-active');
  ...
  reset: function () {
    ...
    const app = document.getElementById('app');
    if (app) {
      app.classList.remove('pregnant-module-active', 'fullscreen-mode');
      app.innerHTML = '';
    }
  },

  // 动态加载 main_pregnant.css（孕妇版专用）
  loadCSS: async function () {
    // 避免重复加载
    if (document.querySelector('link[href*="main_pregnant.css"]')) {
      console.log('[FullscreenManager] main_pregnant.css 已存在，跳过重复加载');
      return;
    }
    ...
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = 'css/main_pregnant.css';  // 请确认你的文件路径是否正确

    return new Promise((resolve, reject) => {
      link.onload = () => {
        console.log('[FullscreenManager] main_pregnant.css 加载成功');
        resolve();
      };
      link.onerror = () => {
        console.error('[FullscreenManager] main_pregnant.css 加载失败');
...
async function loadLanguage(lang = 'de') {
  try {
    ...
    window.LANG = {
      ...
      hint_other_income_annual: 'z. B. ALV, EO (Mutterschaftsentschädigung), IV-Zusatzrente etc.<br><strong>Bitte selbst eintragen</strong> (EO bitte den tatsächlichen Betrag für 98 Tage schätzen)',
      onetime_payment: 'Einmalig'
    };
  }
  document.documentElement.lang = window.LANG.lang || 'de';
}
/* 7. 渲染器 - 孕妇版优化：进入 form 时重置 SH 相关残留数据 - 已修复IPV/EL显示问题 */
function render(step, isBack = false) {
  ...
  if (step === 'form') {
    ...
    // 支持的人群列表（以后扩展时在这里加）
    const supportedCrowds = ['unemployed', 'pregnant'];
    ...
    if (Router.crowd === 'pregnant') {
      console.log('Pregnant form initialized');
      // 默认值
      const pregnancyWeeksInput = document.querySelector('input[name="pregnancyWeeks"]');
      if (pregnancyWeeksInput) pregnancyWeeksInput.value = '20'; // 默认20周
      // 其他默认如 isPregnant=yes 已checked
      // FA：如果pregnancyWeeks >30，显示numNewborns
      updateFAExtraFieldsVisibility(); // 已考虑孕妇
    }

    // ========== 关键修复：只重置用户填写的SH字段，不重置系统计算的IPV/EL字段 ==========
    // 清空或重置 SH 额外字段值（只包含用户主动填写的字段）
    const shFields = ['employmentStatus', 'hasMedicalNeeds', 'other_income_annual', 'monthly_other_expenses'];
    ...
    // ========== 重要：不要删除IPV/EL缓存，否则第一次计算后无法显示！==========
    // 以下两行已被删除，确保IPV/EL缓存保留：
    // delete Router.form.ipvReceivedAnnual;
    // delete Router.form.elReceivedAnnual;

    // 强制重置 pendingSH 和 shExtraShown（防止状态残留）
    Router.pendingSH = false;
    Router.shExtraShown = false;

    console.log('Form page entered: SH extra fields and Router.form SH cache reset (IPV/EL cache preserved)');
    ...
    // ========== 新增：恢复IPV/EL显示值（如果缓存中有） ==========
    // 确保IPV和EL的只读输入框显示正确的值
    if (Router.form.ipvReceivedAnnual !== undefined) {
      const ipvInput = document.querySelector('input[name="ipvReceivedAnnual"]');
      if (ipvInput) {
        ipvInput.value = Router.form.ipvReceivedAnnual.toFixed(2);
        ipvInput.readOnly = true;
        ipvInput.style.backgroundColor = '#f0f8ff';
      }
    }
    if (Router.form.elReceivedAnnual !== undefined) {
      const elInput = document.querySelector('input[name="elReceivedAnnual"]');
      if (elInput) {
        elInput.value = Router.form.elReceivedAnnual.toFixed(2);
...
function bindEvents(step) {
  ...
  if (step === 'form') {
    ...
    // 修改计算按钮逻辑，添加前置检查
    // 修改计算按钮逻辑，添加前置检查（孕妇模式优化版 - 已取消 unemployed 限制，所有人群都检查 SH 必填字段）
    ...
    if (btnCalcOriginal) {
      ...
      btnCalcOriginal.onclick = async () => {
        ...
        // 前置检查2：SH 已显示时验证必填项（孕妇和unemployed都检查，所有人群都检查）
        if (Router.pendingSH || document.getElementById('sh-extra-fields')?.style.display !== 'none') {
          ...
          if (!medicalRadio) {
            errorMessage = errorMessage
              ? errorMessage + '\nBitte geben Sie an, ob medizinische Bedürfnisse bestehen.'
              : 'Bitte geben Sie an, ob medizinische Bedürfnisse bestehen.';
          }
...
async function loadStateRule(st) {
  ...
  try {
    ...
    try {
      ...
      // 显示用户友好的错误信息
      let errorMessage = t('error_load_rules') || `Kann die Berechnungsregeln für ${st} nicht laden. `;
      errorMessage += t('please_check_files') || 'Bitte überprüfen Sie:';
      errorMessage += '\n1. ' + (t('file_ipv_rule') || 'data/ipv/ipshuju_') + st + '.json';
      errorMessage += '\n2. ' + (t('file_ipv_module') || 'js/IPVcalc/ipsuanfa_') + st + '.js';
      errorMessage += '\n3. ' + (t('file_el_rule') || 'data/el/elshuju_') + st + '.json';
      errorMessage += '\n4. ' + (t('file_el_module') || 'js/ELcalc/elsuanfa_') + st + '.js';
      errorMessage += '\n5. ' + (t('file_sh_rule') || 'data/sozialhilfe/SHshuju_') + st + '.json';
      errorMessage += '\n6. ' + (t('file_sh_module') || 'js/SHcalc/SHsuanfa_') + st + '.js';
      errorMessage += '\n7. ' + (t('file_fa_meta') || 'data/fa/fa_') + st + '.json (optional)';
      alert(errorMessage);
      throw altError;
    }
  }
...
function validateForm() {
  ...
  if (!isValid) {
    ...
    if (invalidField) {
      ...
      alert(t('please_fill_field_correctly') || `Bitte füllen Sie "${fieldName}" korrekt aus.`);
      invalidField.focus();
...
function collectForm() {
  ...
  // 失业人员专属字段（收集 + 默认值）
  if (Router.crowd === 'unemployed') {
    Router.form.previousMonthlySalary = Math.max(Number(Router.form.previousMonthlySalary) || 0, 1000);
    Router.form.unemploymentDurationMonths = Number(Router.form.unemploymentDurationMonths) || 6;
    Router.form.hasALV = Router.form.hasALV || 'yes';
    Router.form.hasDisability = Router.form.hasDisability || 'no';
  }

  // 孕妇专属字段（收集 + 默认值 + 逻辑）- 修复版（确保日期被正确捕获）
  if (Router.crowd === 'pregnant') {
    Router.form.isPregnant = Router.form.isPregnant || 'yes';
    Router.form.pregnancyWeeks = Number(Router.form.pregnancyWeeks) || 0;

    // ========== 修复版日期处理：直接从DOM获取原始值，避免依赖Router.form ==========
    const dueDateInput = document.querySelector('input[name="expectedDueDate"]');
    let dueDateRaw = dueDateInput ? dueDateInput.value : '';

    console.log('Raw due date from DOM:', dueDateRaw);

    // 如果用户没有输入日期，尝试从旧表单获取
    if (!dueDateRaw && oldForm.expectedDueDate) {
      dueDateRaw = oldForm.expectedDueDate;
      console.log('Using old form due date:', dueDateRaw);
    }

    // 处理日期格式
    if (dueDateRaw) {
      // 检查是否是 YYYY-MM-DD 格式（浏览器原生 date 输入格式）
      if (/^\d{4}-\d{2}-\d{2}$/.test(dueDateRaw)) {
        const [year, month, day] = dueDateRaw.split('-');
        Router.form.expectedDueDate = `${day}.${month}.${year}`;
        console.log('Converted YYYY-MM-DD to DD.MM.YYYY:', Router.form.expectedDueDate);
      }
      // 检查是否已经是 DD.MM.YYYY 格式
      else if (/^\d{1,2}\.\d{1,2}\.\d{4}$/.test(dueDateRaw)) {
        Router.form.expectedDueDate = dueDateRaw;
        console.log('Already in DD.MM.YYYY format:', Router.form.expectedDueDate);
      }
      // 其他格式，尝试直接保存
      else {
        Router.form.expectedDueDate = dueDateRaw;
        console.log('Saving raw date value:', Router.form.expectedDueDate);
      }
    } else {
      Router.form.expectedDueDate = '-';
      console.log('No due date found, setting to -');
    }

    Router.form.hasPregnancyComplications = Router.form.hasPregnancyComplications || 'no';
    Router.form.pregnancyPreviousSalary = Number(Router.form.pregnancyPreviousSalary) || 0;

    // 逻辑：如果不是孕妇，设置error
    if (Router.form.isPregnant !== 'yes') {
      alert(t('err_not_pregnant') || 'Kein Anspruch für Schwangere: Bitte wählen Sie "Ja".');
      return;
    }

    // 自动加额外需求（官方：孕期+100-200 CHF/月，根据周数和并发症）
    let extraPregnant = Router.form.pregnancyWeeks > 20 ? 150 : 0;
    if (Router.form.hasPregnancyComplications === 'yes') extraPregnant += 200;
    Router.form.zusatzbedarf_monatlich = (Router.form.zusatzbedarf_monatlich || 0) + extraPregnant;

    // 如果临近出生，设置numNewborns=1（影响FA）
    if (Router.form.pregnancyWeeks > 30) {
      Router.form.numNewborns = 1;
    }

    // 【已彻底关闭自动计算】EO 不再自动加入 other_income_annual
    // 完全由用户手动填写（默认 0），最安全做法
    Router.form.other_income_annual = Number(Router.form.other_income_annual) || 0;

    console.log('Pregnant fields collected: EO 自动计算已关闭，用户手动输入生效');
  }
  ...
  if (Router.crowd === 'retired') {
    ...
    if (Router.form.checkEL === 'yes' && Router.form.monthlyPensionAmount === 0) {
      alert(t('el_enter_monthly_pension') || 'Für EL-Berechnung geben Sie bitte den monatlichen Rentenbetrag ein.');
    }
  } else if (Router.crowd === 'student') {
    Router.form.isFulltimeStudent = Router.form.isFulltimeStudent || 'yes';
    Router.form.studyCostsMonthly = Router.form.studyCostsMonthly || 0;
    Router.form.hasStipendium = Router.form.hasStipendium || 'no';
  }
...
function checkPossibleSozialhilfe(inputs, rules) {
  ...
  let assetBuffer = 1.5;
  if (isStudent) assetBuffer = 1.8;
  if (isDisabled) assetBuffer = 2.0;
...
/* 13. 计算（关键修复：IV-Rente桥接必须在EL计算之前，且多字段覆盖） */
async function runCalculation() {
  ...
  if (Router.crowd === 'disabled') {
    ...
    // 如果用户填了IV-Rente，强制覆盖所有可能被EL算法读取的字段
    if (ivMonthly > 0) {
      // 月度字段（各种可能的命名）
      Router.form.monthlyPensionAmount = ivMonthly;
      Router.form.monthlyPension = ivMonthly;
      Router.form.pensionMonthly = ivMonthly;
      Router.form.ivMonthlyPensionConfirmed = ivMonthly; // 额外保险

      // 年度字段（EL算法AG版实际读取的是regularAnnualPension！）
      ...
      // 强制设置EL预检条件为通过（如果有IV-Rente，肯定是领取养老金状态）
      ...
      // 即使IV-Rente为0，也要确保EL算法不会读到undefined
  ...
  // 残疾人通常有IV-Rente，应该允许EL计算，即使资产超标
  ...
  // --- 孕妇额外需求：必须在SH计算前叠加 ---
  // 孕妇额外需求已自动加到 zusatzbedarf_monatlich（collectForm里已有）
  // 这里再确认一次
  if (Router.crowd === 'pregnant') {
    let extra = Router.form.pregnancyWeeks > 20 ? 150 : 0;
    if (Router.form.hasPregnancyComplications === 'yes') extra += 200;
    Router.form.zusatzbedarf_monatlich = (Router.form.zusatzbedarf_monatlich || 0) + extra;
    console.log('[SH] 孕妇额外需求已叠加:', {
      孕周: Router.form.pregnancyWeeks,
      并发症: Router.form.hasPregnancyComplications,
      额外金额: extra,
      累计特殊需求: Router.form.zusatzbedarf_monatlich
    });
  }
  ...
  // --- 孕妇 EO 计算（FA之后、SH之前）---
  if (Router.crowd === 'pregnant') {
    const previousSalary = Number(Router.form.pregnancyPreviousSalary) || 0;

    if (previousSalary > 0) {
      const dailyMax = 220;
      const monthlyMax = dailyMax * 30; // ≈6600
      const eoRate = 0.80;

      let monthlyEO = previousSalary * eoRate;
      if (monthlyEO > monthlyMax) monthlyEO = monthlyMax;

      const totalDays = 98;
      const totalEO = (monthlyEO / 30) * totalDays;

      Router.resultData.eo = {
        eligible: true,
        monthlyBenefit: monthlyEO,
        totalBenefit: totalEO,
        durationDays: totalDays,
        explanation: {
          steps: [
            { label: 'eo_step_previous_salary', value: previousSalary },
            { label: 'eo_step_rate', value: '80%' },
            { label: 'eo_step_monthly_before_cap', value: previousSalary * 0.8 },
            { label: 'eo_step_monthly_max', value: monthlyMax },
            { label: 'eo_step_monthly_actual', value: monthlyEO.toFixed(2) },
            { label: 'eo_step_total_98days', value: totalEO.toFixed(2) + ' CHF' }
          ],
          note_key: 'eo_note_federal'
        }
      };
      ...
      // 完全由用户手动填写（默认 0）
      Router.form.other_income_annual = Number(Router.form.other_income_annual) || 0;
      console.log('[EO] 自动年化已彻底关闭，用户手动输入生效');
    } else {
      Router.resultData.eo = {
        eligible: false,
        reason: t('eo_no_salary')
      };
    }
  }

  // --- ★★★ 强制更新SH输入（即使EL/IPV为0）- 确保SH计算使用最新IPV/EL（减计收入）★★★ ---
  const ipvInputSH = document.querySelector('input[name="ipvReceivedAnnual"]');
  const elInputSH = document.querySelector('input[name="elReceivedAnnual"]');
  if (ipvInputSH) {
    ipvInputSH.value = (Router.form.ipvReceivedAnnual || 0).toFixed(2);
    console.log('[SH-FORCE] IPV年度金额已强制更新:', Router.form.ipvReceivedAnnual);
  }
  if (elInputSH) {
    elInputSH.value = (Router.form.elReceivedAnnual || 0).toFixed(2);
    console.log('[SH-FORCE] EL年度金额已强制更新:', Router.form.elReceivedAnnual);
  }
  ...
  if (!Router.pendingSH) {
    ...
    if (Router.resultData.el && !Router.resultData.el.error && (Router.resultData.el.annualBenefit || 0) > 0) {
      Router.resultData.sozialhilfe = {
        error: 'no_sh_when_el',
        message: t('no_sh_when_el_message')
      };
    } else {
...
function restoreFormData() {
  ...
  document.querySelectorAll('#dynamic-form input, #dynamic-form select').forEach(el => {
    ...
    if (value !== undefined && value !== null) {
      if (el.type === 'checkbox') {
      ...
      } else if (el.type === 'date' && el.name === 'expectedDueDate') {
        // ========== 关键修复：将 DD.MM.YYYY 转换回 YYYY-MM-DD ==========
        if (typeof value === 'string' && value.includes('.') && value.split('.').length === 3) {
          ...
          el.value = value; // 如果是 YYYY-MM-DD 格式或空值，直接使用
        }
      } else {
        el.value = value;
      }
...
function fillResultPage() {
  ...
  // 新增：IV-Rente 卡片（残疾人模式下始终显示）
  if (Router.crowd === 'disabled' && Router.form.ivMonthlyPension > 0) {
    const ivAnnual = Router.form.ivMonthlyPension * 12;
    const ivContainer = document.getElementById('iv-details');

    // 如果容器不存在，先创建
    if (!ivContainer) {
      const ivDetails = document.createElement('details');
      ivDetails.id = 'iv-details';
      ivDetails.className = 'benefit-details';
      ivDetails.innerHTML = `
      <summary class="benefit-summary">
        <span>${t('iv_pension_title') || 'IV-Rente (geschätzt / bekannt)'}</span>
        <span class="benefit-total">
          ${t('annual_short') || 'Jährlich'}: <b id="iv-benefit-annual">${ivAnnual.toFixed(2)}</b> CHF | 
          ${t('monthly_short') || 'Monatlich'}: <b id="iv-benefit-monthly">${Router.form.ivMonthlyPension.toFixed(2)}</b> CHF
      ...
      <div class="benefit-details-content">
        <div class="formula-container">
          <div class="step">
            <span class="label">${t('iv_pension_monthly_label') || 'Bekannte monatliche IV-Rente'}:</span>
            <span class="val">${Router.form.ivMonthlyPension.toFixed(2)} CHF</span>
          </div>
          <div class="step">
            <span class="label">${t('annual_short') || 'Jährlich'} (×12):</span>
            <span class="val">${ivAnnual.toFixed(2)} CHF</span>
          </div>
          <div class="note">
            ${t('iv_pension_note') || 'Dieser Betrag wurde vom Benutzer angegeben und fließt in die EL-Berechnung ein.'}
          </div>
        </div>
      </div>
    `;
      ...
      if (faDetails && faDetails.parentNode) {
        faDetails.before(ivDetails);
      } else {
        document.querySelector('.benefit-details:last-of-type')?.parentNode?.appendChild(ivDetails);
      }
    } else {
      // 已存在 → 只更新金额
      document.getElementById('iv-benefit-annual').textContent = ivAnnual.toFixed(2);
      document.getElementById('iv-benefit-monthly').textContent = Router.form.ivMonthlyPension.toFixed(2);
    }
  }
  ...
  // 4.5 EO（Mutterschaftsentschädigung）- 强制显示，即使为0也要说明原因
  const eoContainer = document.getElementById('eo-details');
  if (eoContainer) {
    if (Router.crowd === 'pregnant') {  // 只在孕妇模式显示
      eoContainer.style.display = 'block';  // 强制显示

      if (b.eo && b.eo.eligible) {
        // 有资格，显示金额
        const eoTotal = document.getElementById('eo-total-benefit');
        const eoMonthly = document.getElementById('eo-monthly-benefit');
        if (eoTotal) eoTotal.textContent = formatCurrency(b.eo.totalBenefit || 0);
        if (eoMonthly) eoMonthly.textContent = formatCurrency(b.eo.monthlyBenefit || 0);
        showFormula(b.eo, 'eo-formula-box');
      } else {
        // 无资格，显示原因
        const eoTotal = document.getElementById('eo-total-benefit');
        const eoMonthly = document.getElementById('eo-monthly-benefit');
        if (eoTotal) eoTotal.textContent = '0.00';
        if (eoMonthly) eoMonthly.textContent = '0.00';

        const formulaBox = document.getElementById('eo-formula-box');
        if (formulaBox) {
          const reasonText = b.eo?.reason || t('eo_no_salary') || 'Bruttolohn vor Mutterschaft nicht angegeben oder 0 CHF.';
          formulaBox.innerHTML = `
            <div class="warning-box" style="display:block; background:#f8d7da; color:#721c24; border-color:#f5c6cb; padding:15px; margin-bottom:15px;">
              <strong>${t('eo_no_entitlement_title') || 'Kein Anspruch auf Mutterschaftsentschädigung (EO)'}</strong><br>
              ${reasonText}
            </div>
          `;
        }
  ...
  if (Router.crowd === 'unemployed' && Router.resultData.alv) {
    ...
    if (!alvContainer) {
      ...
      if (hint) {
        hint.textContent = `(${t('details_expand')})`;
      }
...
function displayUserInputs() {
  ...
  // === 孕妇专属输入字段 ===
  if (Router.crowd === 'pregnant') {
    if (inputs.isPregnant !== undefined) {
      html += `<tr><td>${t('label_is_pregnant') || 'Sind Sie schwanger?'}:</td><td>${inputs.isPregnant === 'yes' ? t('yes') : t('no')}</td></tr>`;
    }
    if (inputs.pregnancyWeeks !== undefined) {
      html += `<tr><td>${t('label_pregnancy_weeks') || 'Schwangerschaftswoche'}:</td><td>${inputs.pregnancyWeeks}</td></tr>`;
    }
    if (inputs.expectedDueDate !== undefined && inputs.expectedDueDate !== '') {
      html += `<tr><td>${t('label_expected_due_date') || 'Erwartetes Geburtsdatum'}:</td><td>${inputs.expectedDueDate}</td></tr>`;
    } else {
      html += `<tr><td>${t('label_expected_due_date') || 'Erwartetes Geburtsdatum'}:</td><td>-</td></tr>`;
    }
    if (inputs.hasPregnancyComplications !== undefined) {
      html += `<tr><td>${t('label_has_pregnancy_complications') || 'Schwangerschaftskomplikationen'}:</td><td>${inputs.hasPregnancyComplications === 'yes' ? t('yes') : t('no')}</td></tr>`;
    }
    if (inputs.pregnancyPreviousSalary !== undefined) {
      html += `<tr><td>${t('label_pregnancy_previous_salary') || 'Monatslohn vor Mutterschaft'}:</td><td>${formatCurrency(inputs.pregnancyPreviousSalary)} CHF</td></tr>`;
    }
  }
...
async function showFormula(b, boxId) {
  ...
  if (boxId.includes('fa') && (!b || b.error)) {
    ...
    if (Router.crowd === 'single') {
      ...
      let errorMessage = t(errorKey);
      if (errorMessage === errorKey) errorMessage = t('err_general_no_entitlement');
      errorHtml += `<strong>${t('error')}:</strong> ${errorMessage}`;
    }
    errorHtml += '</div>';
  ...
  else if (boxId.includes('eo')) benefitType = 'eo';

  let rule = null;

  // 根据benefitType选择数据源 + ALV/EO特殊处理
  if (benefitType === 'alv') {
    // ALV 不从RULE加载，使用固定联邦信息（全部用翻译键）
    html += '<div class="legal-info-section">';
    html += `<h4>${t('legal_basis_title') || t('legal_basis_title')}</h4>`;

    html += '<div class="application-card">';
    html += `<h5>${t('how_to_apply') || t('how_to_apply')}</h5>`;
    ...
    // 所需文件（从lang.json数组读取）
    const alvDocKey = getDocumentKey(state, 'alv');
    let alvDocs = [];
    ...
    // 法律依据 + 链接
    html += `<p><strong>${t('legal_basis') || 'Rechtsgrundlage'}:</strong> ${t('eo_legal_basis') || 'Erwerbsersatzordnung (EOG), Art. 16b–16h ATSG'}</p>`;
    html += `<p><strong>Gesetzestext:</strong> <a href="${t('eo_legal_source_url') || 'https://www.fedlex.admin.ch/eli/cc/2004/564/de'}" target="_blank" style="color:#007bff;">Erwerbsersatzgesetz (EOG) – fedlex.admin.ch</a></p>`;

    html += `<h5>${t('how_to_apply') || t('how_to_apply')}</h5>`;

    // 获取所需文件数组
    const eoDocs = window.LANG && Array.isArray(window.LANG.eo_required_documents)
      ? window.LANG.eo_required_documents
      : [];

    html += `
    <p><strong>${t('eo_application_authority') || 'Ihre AHV-Ausgleichskasse (meist über den Arbeitgeber oder direkt bei der AHV)'}</strong></p>
    <p><strong>${t('eo_official_info') || 'Offizielle Informationen zur Mutterschaftsentschädigung'}:</strong> <a href="${t('eo_official_website') || 'https://www.ahv-iv.ch/de'}" target="_blank" style="color:#007bff;">${t('eo_official_website') || 'www.ahv-iv.ch'}</a></p>
    <p><strong>${t('required_documents') || 'Benötigte Unterlagen'}:</strong></p>
    ...
    if (eoDocs.length > 0) {
      ...
      // 后备默认值
      html += `
      <li>✓ ${t('eo_doc_birth_certificate') || 'Geburtsmeldung / Arztzeugnis'}</li>
      <li>✓ ${t('eo_doc_salary_statements') || 'Lohnbescheinigungen der letzten 9–12 Monate'}</li>
      <li>✓ ${t('eo_doc_employer_confirmation') || 'Arbeitgeberbestätigung'}</li>
      <li>✓ ${t('eo_doc_ahv_number') || 'AHV-Nummer'}</li>
      `;
    }
  ...
  // 下面是原有的 rule 存在时的渲染逻辑（ALV/EO已在上方单独处理，所以跳过）
  if (rule && benefitType !== 'alv' && benefitType !== 'eo') {
    html += '<div class="legal-info-section" style="margin-top:20px; padding-top:15px; border-top:1px solid #dee2e6;">';
    html += `<h4>${t('legal_basis_title') || 'Rechtliche Grundlagen & Informationen'}</h4>`;
    ...
    html += `<p><strong>${t('phone') || 'Tel'}:</strong> ${phone}</p>`;
    html += `<p><strong>${t('email') || 'Email'}:</strong> ${email !== 'Nicht angegeben' ? `<a href="mailto:${email}">${email}</a>` : email}</p>`;

    const officialUrl = benefitType === 'fa' ? rule.application?.url : (rule.application?.url || rule.official_url);
    if (officialUrl) {
      html += `<p><strong>${t('official_website') || 'Offizielle Webseite'}:</strong> <a href="${officialUrl}" target="_blank" style="color:#007bff;">${officialUrl}</a></p>`;
    }

    // 所需文件
    let docs = [];
    // 对于 ALV 和 EO，已经在前面单独处理了，这里不需要再处理
    if (benefitType !== 'alv' && benefitType !== 'eo') {
      // 使用 getDocumentKey 生成文档键名
    ...
    if (noteText && noteText !== noteKey) {
      html += `<div class="note-hint" style="margin-top:15px; background:#fff3cd; padding:10px; border:1px solid #ffeeba; border-radius:4px;"><strong>${t('important_note') || 'Wichtiger Hinweis'}:</strong> ${noteText}</div>`;
    }

    html += '</div></div>';
  } else if (benefitType !== 'alv' && benefitType !== 'eo') {
    // 非ALV/EO且rule不存在时的兜底
...
<body>
    ...
    <div class="section">
        ...
        <table class="info-table">
            ...
            <tr>
              <td>Andere jährliche Einkünfte (z. B. ALV, EO Mutterschaftsentschädigung, IV-Zusatzrente etc.) CHF<br>
              <small><strong>Bitte selbst eintragen</strong> (EO bitte den tatsächlichen Betrag für 98 Tage schätzen)</small></td>
              <td>${formatCurrency(form.other_income_annual || 0)} CHF</td>
            </tr>
...
/* 20. PDF 生成 — 严格按照家庭版格式修改，同时保留孕妇版专属内容 */
async function generatePDF() {
  ...
  try {
    ...
    const LINE_HEIGHT = 5.5;    // 固定行高，与家庭版一致
    ...
    const drawSection = (title) => {
      ...
      pdf.text(safeTitle, pageWidth / 2, yPos, { align: 'center' });   // ← 改成居中！
      yPos += 6;
      pdf.line(margin, yPos, pageWidth - margin, yPos);
      yPos += 8;
    };
    ...
    const drawApplicationInfo = async (type, state) => {
      ...
      if (type === 'eo') {
        appInfo = {
          authority: t('eo_application_authority') || 'Ihre AHV-Ausgleichskasse (meist über den Arbeitgeber oder direkt bei der AHV)',
          website: t('eo_website') || 'https://www.ahv-iv.ch/de',
          note: t('eo_note') || 'EO ist eine Bundesleistung. Beantragen Sie bei Ihrer AHV-Ausgleichskasse oder über den Arbeitgeber.'
        };
        const eoDocKey = 'eo_required_documents';
        if (window.LANG && Array.isArray(window.LANG[eoDocKey])) {
          documents = window.LANG[eoDocKey];
        } else {
          documents = [
            '✓ Geburtsmeldung / Arztzeugnis',
            '✓ Lohnbescheinigungen der letzten 9–12 Monate',
    ...
    const drawBenefitCard = async (type, title, result) => {
      ...
      if (type === 'eo' && result.durationDays) cardHeight += 5;
      ...
      if (hasError) {
        ...
        // EO特殊处理：使用修改后的新代码
        if (type === 'eo') {
          const totalAmount = result.totalBenefit || annual;
          const monthlyAmount = result.monthlyBenefit || monthly;
          let titleHeight = titleLineCount * LINE_HEIGHT;
          let amountY = yPos + 7 + titleHeight + 5;

          // 总金额
          pdf.setFontSize(11);
          pdf.setFont(FONT_FAMILY, FONT_BOLD);
          pdf.setTextColor(40, 167, 69);
          const totalText = `${t('eo_total_title')}: ${formatCurrency(totalAmount)} CHF`;
          pdf.text(processTextForPDF(totalText), margin + 8, amountY);

          // 月金额
          if (hasAmount) {
            pdf.setFontSize(9);
            pdf.setTextColor(100, 100, 100);
            const monthlyText = `${t('eo_monthly_approx')}: ${formatCurrency(monthlyAmount)} CHF`;
            pdf.text(processTextForPDF(monthlyText), margin + 8, amountY + 5);
          }
          ...
          pdf.setFontSize(9);
          pdf.setTextColor(100, 100, 100);
          const durationText = `${t('eo_duration')}: ${result.durationDays} Tage (ca. 14 Wochen)`;
          pdf.text(processTextForPDF(durationText), margin + 8, yPos);
          yPos += 8;
        } else {
          // 其他福利类型的正常显示
          let amountText;
          if (type === 'eo') {
            amountText = `${t('eo_total_title') || 'Gesamt (98 Tage)'}: ${formatCurrency(result.totalBenefit || annual)} CHF`;
          } else {
          ...
          if (hasAmount) {
            ...
            let monthlyText;
            if (type === 'eo') {
              monthlyText = `${t('eo_monthly_approx') || 'ca. monatlich'}: ${formatCurrency(result.monthlyBenefit || monthly)} CHF`;
            } else {
              monthlyText = `${t('monthly_short') || 'Monatlich'}: ${formatCurrency(monthly)} CHF`;
            }
        ...
        // 绘制EO额外信息 - 这部分已经在EO特殊处理中完成，所以这里可以注释掉或保留但不会执行
        // if (type === 'eo' && result.durationDays) {
        //   yPos += 3;
        //   pdf.setFontSize(9);
        //   pdf.setTextColor(100, 100, 100);
        //   const durationText = processTextForPDF(t('eo_duration') || `Dauer: ${result.durationDays} Tage (ca. 14 Wochen)`, true);
        //   pdf.text(durationText, margin + 8, yPos);
        //   yPos += 5;
        // }
      }
    ...
    // ---------- 第一部分：Ihre Eingabedaten ----------
    drawSection(t('input_data') || 'Ihre Eingabedaten');

    const form = Router.form || {};
    const crowd = Router.crowd || 'pregnant';
    ...
    if (crowd === 'pregnant') {
      inputFields.push(
        [t('label_is_pregnant') || 'Sind Sie schwanger?', form.isPregnant === 'yes' ? t('yes') : t('no')],
        [t('label_pregnancy_weeks') || 'Schwangerschaftswoche', String(form.pregnancyWeeks || '0')],
        [t('label_expected_due_date') || 'Erwartetes Geburtsdatum', form.expectedDueDate || '-'],
        [t('label_has_pregnancy_complications') || 'Schwangerschaftskomplikationen', form.hasPregnancyComplications === 'yes' ? t('yes') : t('no')],
        [t('label_pregnancy_previous_salary') || 'Monatslohn vor Mutterschaft', formatCurrency(form.pregnancyPreviousSalary || 0) + ' CHF']
      );
    }
    ...
    if (crowd === 'disabled') {
      inputFields.push(
        [t('label_invalidity_degree') || 'Invaliditätsgrad', (form.invalidityDegree || '0') + '%'],
        [t('label_iv_monthly_pension') || 'Monatliche IV-Rente', formatCurrency(form.ivMonthlyPension || 0) + ' CHF']
      );
      if (form.hilflosenLevel) {
        let hilflosenText = '';
        if (form.hilflosenLevel === 'leicht') hilflosenText = t('hilflosen_leicht') || 'Leicht (252 CHF/Monat)';
        else if (form.hilflosenLevel === 'mittel') hilflosenText = t('hilflosen_mittel') || 'Mittel (630 CHF/Monat)';
        else if (form.hilflosenLevel === 'schwer') hilflosenText = t('hilflosen_schwer') || 'Schwer (1008 CHF/Monat)';
        inputFields.push([t('label_hilflosen_level') || 'Hilflosenentschädigung Stufe', hilflosenText]);
      }
    }
    ...
    if (crowd === 'unemployed') {
      inputFields.push(
        [t('label_previous_monthly_salary') || 'Monatslohn vor Arbeitslosigkeit', formatCurrency(form.previousMonthlySalary || 0) + ' CHF'],
        [t('label_unemployment_duration_months') || 'Erwartete Arbeitslosigkeitsdauer', (form.unemploymentDurationMonths || '0') + ' Monate'],
        [t('label_has_alv') || 'Bezieht ALV', form.hasALV === 'yes' ? t('yes') : t('no')],
        [t('label_has_disability') || 'Anerkannte Behinderung', form.hasDisability === 'yes' ? t('yes') : t('no')]
      );
    }
    ...
    if (form.monthly_other_expenses > 0) {
    ...
    // 单列渲染所有字段 - 使用家庭版的行距计算方式
    pdf.setFontSize(10);
    inputFields.forEach(([label, value]) => {
      checkPageBreak(10); // 家庭版用的10
      ...
      // 更新yPos到标签之后 - 使用LINE_HEIGHT，不加额外间距
      ...
      // 更新yPos到数值之后 - 使用LINE_HEIGHT，不加额外间距
      yPos += (valueLines.length * LINE_HEIGHT);
    });

    // 在所有字段之后增加一个LINE_HEIGHT的间距
    yPos += LINE_HEIGHT;
    ...
    // ALV
    if (crowd === 'unemployed' && results.alv) {
    ...
    // EO - 孕妇专属
    if (crowd === 'pregnant' && results.eo) {
      await drawBenefitCard('eo', t('eo_title') || 'Mutterschaftsentschädigung (EO)', results.eo);
    }
...
function getErrorMessage(errorObj) {
  ...
  if (typeof errorObj === 'string') {
    ...
    if (errorObj.includes('not_possible_sh')) return 'Kein Anspruch auf Sozialhilfe.';
  ...
  const htmlContent = `
    ...
    <body>
      <div class="header">
        ...
        <p><em>${t('button_pdf') || 'Ergebnisse als PDF speichern'}</em></p>
      </div>
...
/* 23. 模板函数 - 现在默认显示孕妇版欢迎词 + 默认选中低收入 */
const tmplCrowd = () => `
  <h2>${t('welcome_pregnant')}</h2>
  <p style="color:#666; margin-bottom:20px;">
    ${t('pregnant_welcome_text') || 'Bitte geben Sie Ihre Schwangerschaftsdetails und finanzielle Situation ein. Dies beeinflusst vor allem mögliche Mutterschaftsentschädigung, Familienzulagen (FA) und Sozialhilfe (SH).'}
  </p>
  <select id="sel-crowd">
    <option value="">-- ${t('select_crowd')} --</option>
    <option value="pregnant" selected>${t('crowd_pregnant')}</option>
  </select>
...
const tmplUnemployed = () => `
  ...
  <div class="mode-notice">
    <strong>${t('pregnant_mode_active')}</strong>
    ${t('pregnant_hint')}
  </div>
  <form id="dynamic-form">
    <!-- 基本财务信息（共用） -->
    <label>${t('annual_income')} (CHF)</label>
    <span class="hint">${t('yunfu_hint_annual_income')}</span>
    ...
    <input name="numChildren" type="number" min="0" value="0" placeholder="${t('placeholder_num_children') || 'z.B. 0'}">
    
    <label>${t('num_education')}</label>
    <span class="hint">${t('hint_num_education')}</span>
    <input name="numEducation" type="number" min="0" value="0" placeholder="${t('placeholder_num_education') || 'z.B. 0'}">
    ...
    <!-- 成人数量固定为1 -->
    <label>${t('num_adults')} (${t('fixed') || 'fest'}: 1)</label>
    <input name="numAdults" type="number" min="1" max="1" value="1" readonly style="background:#f0f8ff;">
    
    <!-- 孕妇专属字段 -->
    <div style="background:#f8f9fa; padding:15px; border:1px solid #dee2e6; border-radius:4px; margin-top:20px;">
      <h4>${t('pregnant_extra_title')}</h4>
      
      <label>${t('label_is_pregnant')}</label>
      <span class="hint">${t('hint_is_pregnant')}</span>
      <div style="margin:5px 0 15px;">
        <label style="display:inline-block; margin-right:20px;">
          <input type="radio" name="isPregnant" value="yes" checked> ${t('yes')}
        </label>
        <label style="display:inline-block;">
          <input type="radio" name="isPregnant" value="no"> ${t('no')}
        </label>
      </div>
      
      <label>${t('label_pregnancy_weeks')}</label>
      <span class="hint">${t('hint_pregnancy_weeks')}</span>
      <input name="pregnancyWeeks" type="number" min="0" max="40" placeholder="${t('placeholder_pregnancy_weeks') || 'z.B. 25'}" value="0" required>
      
      <label>${t('label_expected_due_date')}</label>
      <span class="hint">${t('hint_expected_due_date')}</span>
      <input name="expectedDueDate" type="date" required>
      
      <label>${t('label_has_pregnancy_complications')}</label>
      <span class="hint">${t('hint_has_pregnancy_complications')}</span>
      <div style="margin:5px 0 15px;">
        <label style="display:inline-block; margin-right:20px;">
          <input type="radio" name="hasPregnancyComplications" value="yes"> ${t('yes')}
        </label>
        <label style="display:inline-block;">
          <input type="radio" name="hasPregnancyComplications" value="no" checked> ${t('no')}
        </label>
      </div>
      
      <label>${t('label_pregnancy_previous_salary')}</label>
      <span class="hint">${t('hint_pregnancy_previous_salary')}</span>
      <input name="pregnancyPreviousSalary" type="number" step="0.01" min="0" placeholder="z.B. 4500" required>
    </div>
    ...
    <div id="el-extra-fields" style="display:none">
      ...
      <div id="el-other-fields">
        ...
        <input name="residenceYears" type="number" min="0" max="100" placeholder="${t('placeholder_residence_years') || 'z.B. 8'}">
      </div>
    </div>
    
    <!-- ========== SH 额外字段 - 已添加IPV/EL只读显示框 ========== -->
    <div id="sh-extra-fields" style="display:block; margin-top:20px; padding:15px; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px;">
      ...
      <span class="hint">${t('hint_employment_status')}</span>
      ...
      <label style="margin-top:15px;">${t('label_has_medical_needs')}</label>
      <span class="hint">${t('hint_has_medical_needs')}</span>
      <div style="margin:5px 0 15px;">
        <label style="display:inline-block; margin-right:20px;">
          <input type="radio" name="hasMedicalNeeds" value="yes"> ${t('yes')}
        </label>
        <label style="display:inline-block;">
          <input type="radio" name="hasMedicalNeeds" value="no" checked> ${t('no')}
        </label>
      </div>

      <label>${t('label_zusatzbedarf_monatlich')}</label>
      <span class="hint">${t('hint_zusatzbedarf_monatlich')}</span>
      <input name="zusatzbedarf_monatlich" type="number" min="0" value="0" placeholder="${t('placeholder_zusatzbedarf_monatlich') || 'z.B. 200'}">

      <label>${t('label_other_income_annual')}</label>
      <span class="hint">${t('hint_other_income_annual')}</span>
      <input name="other_income_annual" type="number" min="0" value="0">

      <label>${t('label_monthly_other_expenses')}</label>
      <input name="monthly_other_expenses" type="number" min="0" value="0">

      <label style="color: #28a745; margin-top:20px;">✓ ${t('label_ipv_received_annual')}</label>
      <input name="ipvReceivedAnnual" type="number" step="0.01" min="0" placeholder="0.00" value="0" 
             readonly style="background-color: #f0f8ff; border-color: #28a745; font-weight: bold;">

      <label style="color: #28a745; margin-top:15px;">✓ ${t('label_el_received_annual')}</label>
      <input name="elReceivedAnnual" type="number" step="0.01" min="0" placeholder="0.00" value="0" 
             readonly style="background-color: #f0f8ff; border-color: #28a745; font-weight: bold;">
    </div>
  </form>
  ...
  <!-- SH 二次计算提示（默认隐藏，共用） -->
  <div id="sh-recalc-hint" class="warning-box" style="display:none; background:#fff3cd; border:2px solid #ffeeba; color:#856404; padding:18px; margin-top:25px; font-size:1.1em; font-weight:bold; text-align:center; border-radius:8px;">
...
const tmplResult = () => `
  ...
  <details class="benefit-details" id="eo-details">
    <summary class="benefit-summary">
      <span>${t('eo_title')}</span>
      <span class="benefit-total">
        ${t('eo_total_title') || 'Gesamt (98 Tage)'}: <b id="eo-total-benefit">0.00</b> CHF | 
        ${t('eo_monthly_approx') || 'ca. monatlich'}: <b id="eo-monthly-benefit">0.00</b> CHF
        <span class="toggle-hint">(${t('details_expand')})</span>
      </span>
    </summary>
    <div class="benefit-details-content">
      <div id="eo-formula-box" class="formula-container"></div>
    </div>
  </details>
  <!-- SH -->
  ...
  <!-- ALV（失业专属）- 动态创建，但为了完整性保留 -->
  <details class="benefit-details" id="alv-details" style="display:none">
    <summary class="benefit-summary">
      <span>${t('alv_title') || 'Arbeitslosenversicherung (ALV)'}</span>
      <span class="benefit-total">
        ${t('annual_short')}: <b id="alv-benefit-annual">0.00</b> CHF | 
        ${t('monthly_short')}: <b id="alv-benefit-monthly">0.00</b> CHF
        <span class="toggle-hint">(${t('details_expand')})</span>
      </span>
    </summary>
    <div class="benefit-details-content">
...
window.initCrowdModule = async function () {  // ← 添加 async
  console.log('Initializing pregnant crowd module (fullscreen mode)');

  try {
    // 1. 加载孕妇版专用CSS（如果有）
    await FullscreenManager.loadCSS().catch(e => {
      console.warn('Pregnant CSS load failed, continue anyway', e);
    });
    ...
    if (app) {
      app.innerHTML = '';
      app.classList.add('pregnant-module-active');  // 添加模块激活类
    }

    // 7. 开始渲染（从州选择页面开始）
    render('state');

    console.log('Pregnant fullscreen module initialized successfully');

  } catch (error) {
    console.error('Pregnant module init failed:', error);
    ...
    if (app) {
      app.innerHTML = `
        <div style="color: red; text-align: center; padding: 50px;">
          <h3>${t('pregnant_module_load_error') || '孕妇模块加载失败'}</h3>
          <p>${error.message}</p>
          <button onclick="location.reload()">${t('reload') || '重新加载'}</button>
        </div>
      `;
    }
  }
...
// ========== 添加这一行 - 标记是否通过loader加载 ==========
window.isModuleLoadedByLoader = window.location.search.includes('crowd=');

// ========== 修改后的window.onload ==========
window.onload = async function () {
  // 检查是否通过loader加载（URL中有crowd=参数）
  if (window.isModuleLoadedByLoader) {
    // 通过loader加载 - 什么都不做，等待loader调用initCrowdModule
    console.log('Pregnant module loaded by loader, waiting for init...');
    return;
  } else {
    // 直接访问此文件时，执行原来的初始化逻辑
    console.log('Direct access to pregnant module, running original onload');

    // 这里是原来的初始化代码
    document.body.innerHTML = '<div id="app"></div>';
    await loadLanguage();
    addStyles();
    Router.crowd = 'pregnant';  // 或者你想要默认的人群
    render('state');
  }
};

console.log('Pregnant module loaded, ready for initialization');
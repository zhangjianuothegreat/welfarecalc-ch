...
const BENEFIT_TYPE_MAP = {
  ...
  'alv': 'alv'
  // 未来扩展
  // 'iv': 'iv' 或 'invaliditaet' - 需要根据语言文件决定
};
...
function t(key, fallbackToKey = true) {
  ...
  if (value === undefined && key.includes('_')) {
    ...
    // IPV特殊处理
    if (key.includes('_ipv_authority')) {
      variants.push(key.replace('_ipv_authority', '_ipv_application_authority'));
    }
  ...
  return value !== undefined ? value : (fallbackToKey ? key : '');
}

/**
 * 获取标准化后的福利类型名称
 * @param {string} type - 原始类型（可能是简写或全称）
 * @returns {string} 标准化后的类型名称（用于键名拼接）
 */
function getNormalizedBenefitType(type) {
  // 如果映射表中有对应关系，返回全称
  if (BENEFIT_TYPE_MAP[type]) {
    return BENEFIT_TYPE_MAP[type];
  }

  // 如果已经是全称（或不在映射表中），原样返回
  return type;
}
 ...
 * @param {string} state - 州代码
 * @param {string} type - 福利类型
 * @returns {string[]} 优先键名 + 降级键名数组
 */
function getDocumentKeyWithFallback(state, type) {
  const normalized = getNormalizedBenefitType(type);
  const primaryKey = `${state}_${normalized}_required_documents_list`;

  // 降级方案
  const fallbacks = [
    primaryKey,
    `${state}_${type}_required_documents_list`,  // 原简写形式
    `${normalized}_required_documents_list`,      // 无州前缀
    `${type}_required_documents_list`             // 无州前缀+简写
  ];
...
const FullscreenManager = {
  ...
  enter: function () {
    ...
    app.classList.add('fullscreen-mode', 'retired-module-active');
  ...
  exit: function () {
    ...
    app.classList.remove('fullscreen-mode', 'retired-module-active');
  ...
  reset: function () {
    ...
    const app = document.getElementById('app');
    if (app) {
      app.classList.remove('retired-module-active', 'fullscreen-mode');
      app.innerHTML = '';
    }
  },

  // 动态加载 main_retired.css（退休版专用）
  loadCSS: async function () {
    // 避免重复加载
    if (document.querySelector('link[href*="main_retired.css"]')) {
      console.log('[FullscreenManager] main_retired.css 已存在，跳过重复加载');
      return;
    }
    ...
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = 'css/main_retired.css';  // 请确认你的文件路径是否正确

    return new Promise((resolve, reject) => {
      link.onload = () => {
        console.log('[FullscreenManager] main_retired.css 加载成功');
        resolve();
      };
      link.onerror = () => {
        console.error('[FullscreenManager] main_retired.css 加载失败');
...
window.onload = async () => {
  ...
  if (!isLoadedByLoader) {
    console.log('Direct access to retired module, initializing...');
    ...
    console.log('Retired module loaded by loader, waiting for init...');
  }
};
...
async function loadLanguage(lang = 'de') {
  try {
    ...
    // ========== 添加语言文件验证 ==========
    console.log('[LANG VALIDATION] Checking key consistency...');

    // 随机检查几个州的关键键名
    const testStates = ['AG', 'ZH', 'BE'];
    const testTypes = ['sozialhilfe', 'ipv', 'el', 'fa'];

    testStates.forEach(state => {
      testTypes.forEach(type => {
        const key = `${state}_${type}_required_documents_list`;
        if (window.LANG[key]) {
...
function render(step, isBack = false) {
  ...
  if (step === 'form') {
    console.log('Using form template:', Router.crowd === 'retired' ? 'tmplRetired' : 'tmplForm');
  }
...
function collectForm() {
  ...
  // ========== 关键修改：不再硬设置 numAdults = 1 ==========
  // 只设置未定义的人口字段默认值
  if (Router.form.numAdults === undefined) Router.form.numAdults = 1;  // 只有未定义时才设为1
  if (Router.form.numChildren === undefined) Router.form.numChildren = 0;
  if (Router.form.numEducation === undefined) Router.form.numEducation = 0;
  if (Router.form.numNewborns === undefined) Router.form.numNewborns = 0;
  if (Router.form.numAdoptions === undefined) Router.form.numAdoptions = 0;
  if (Router.form.numChildrenOver12 === undefined) Router.form.numChildrenOver12 = 0;
  if (Router.form.numEducationOver18 === undefined) Router.form.numEducationOver18 = 0;
  // 注意：numAdults 已经通过表单收集，这里不再强制设置为1
  // =====================================================
  ...
  if (Router.crowd === 'retired') {
    ...
    const annualPension = Router.form.monthlyPensionAmount * 12;

    // === 关键：提供两个标准化字段（所有州算法都应该优先读取）===
    Router.form.annualPension = annualPension;               // 12个月常规部分
    Router.form.regularAnnualPension = annualPension;        // 明确是12个月（供2026年第13个月中性化使用）

    // 保留 monthlyPensionAmount 给旧算法兼容
    Router.form.monthlyPensionAmount = Router.form.monthlyPensionAmount;

    // EL 强制如果 checkEL=yes，必须有养老金
    if (Router.form.checkEL === 'yes' && Router.form.monthlyPensionAmount === 0) {
      alert(t('retired_el_pension_required'));
    }
  ...
  if (!Router.pendingSH) { // 第一次进来
    if (checkPossibleSozialhilfe(Router.form, stateRules)) {
      ...
      if (userWantsSH) {
        ...
        return Router.resultData; // 直接返回结果（修复非法 return）
      }
    } else {
    ...
    if (Router.resultData.el && !Router.resultData.el.error && (Router.resultData.el.annualBenefit || 0) > 0) {
      ...
      // 准备 SH 计算用的临时输入对象（不修改原 Router.form）
      ...
      if (shInput.crowd === 'student') {
        ...
        console.log('Student extra added to SH temp input');
      }
      // 退休版额外调整
      if (shInput.crowd === 'retired') {
        if (shInput.hasMedicalNeeds === 'yes') {
          shInput.monthly_other_expenses = (shInput.monthly_other_expenses || 0) + 200; // 老人护理额外
          console.log('Retired extra added: 200 CHF for medical needs');
        }
      }
      // 使用临时输入对象计算 SH
      try {
    ...
    Router.pendingSH = false;
    Router.shExtraShown = false;
    return Router.resultData; // 最后返回结果
  }
  // --- 流程 D: FA 计算 ---
  // ... (保持不变)
  // 最终日志
  // ... (保持不变)
  return Router.resultData; // 确保函数末尾有 return
}
...
function fillResultPage() {
  ...
  showFormula(b.ipv, 'ipv-formula-box'); // 调整：放在fillBenefitAmount之后立即调用
  ...
  if (Router.form.checkEL === 'yes' && b.el && !b.el.error?.includes('skipped')) {
    ...
    showFormula(b.el, 'el-formula-box'); // 调整：放在fillBenefitAmount之后立即调用
  } else {
  ...
  fillFAOneTime(b.fa);
  showFormula(b.fa, 'fa-formula-box'); // 调整：放在fillFAOneTime之后立即调用
  ...
  if (b.sozialhilfe) {
    ...
    if (b.sozialhilfe.error === 'no_sh_when_el') {
      ...
      showFormula(b.sozialhilfe, 'sh-formula-box'); // 调整：放在fillBenefitAmount之后立即调用
    } else {
      // 其他错误情况
      document.getElementById('sh-benefit-annual').textContent = '0.00';
      document.getElementById('sh-benefit-monthly').textContent = '0.00';
...
function displayUserInputs() {
  ...
  html += `<h3>${t('input_data') || 'Eingabeinformationen'}</h3>`;
  ...
  // 新增：学生专属信息（只在学生模式显示）
  if (inputs.crowd === 'student') {
    html += `<tr><td>${t('is_fulltime_student')}:</td><td>${inputs.isFulltimeStudent === 'yes' ? t('yes_fulltime') : t('parttime')}</td></tr>`;
    html += `<tr><td>${t('study_costs_monthly')}:</td><td>${formatCurrency(inputs.studyCostsMonthly || 0)} CHF</td></tr>`;
    html += `<tr><td>${t('has_stipendium')}:</td><td>${inputs.hasStipendium === 'yes' ? t('yes') : t('no')}</td></tr>`;
  }
  // 新增：退休专属信息
  if (inputs.crowd === 'retired') {
    html += `<tr><td>${t('monthly_pension')}</td><td>${formatCurrency(inputs.monthlyPensionAmount)} CHF</td></tr>`;
  }
...
async function showFormula(b, boxId) {
  ...
  if (rule) {
    ...
    // 使用带降级的键名获取
    const docKeys = getDocumentKeyWithFallback(state, benefitType);
    let foundKey = null;

    // 尝试所有可能的键名
    for (const key of docKeys) {
      if (window.LANG && Array.isArray(window.LANG[key])) {
        docs = window.LANG[key];
        foundKey = key;
        console.log(`[DOCS] Found for ${benefitType} using key: ${key}`);
        break;
      }
    }

    // 如果没找到，尝试从 rule 对象获取
    if (docs.length === 0 && rule && rule.required_documents) {
      if (Array.isArray(rule.required_documents)) {
        docs = rule.required_documents;
      } else if (rule.required_documents.list_key) {
        const listKey = rule.required_documents.list_key;
        if (window.LANG && Array.isArray(window.LANG[listKey])) {
          docs = window.LANG[listKey];
        }
      }
    }
...
<body>
    <div class="header">
        ...
        <div class="subtitle">
            ${t('pdf_subtitle') || 'Berechnungsergebnisse für'} ${stateName || 'Schweiz'} (${state || 'CH'}) - ${t('pdf_calculation_date') || 'Erstellt am'} ${now}
        </div>
    </div>

    <!-- 用户信息部分 -->
    <div class="section">
        <div class="section-title">${t('input_data') || 'Eingabedaten'}</div>
        <table class="info-table">
            ...
            <tr>
                <td>${t('zielgruppe') || 'Personengruppe'}</td>
                <td>${t('crowd_' + Router.crowd) || Router.crowd || t('not_specified') || 'Nicht angegeben'}</td>
                <td>${t('canton')}</td>
                <td>${stateName || t('not_specified') || 'Nicht angegeben'}</td>
            </tr>
            <tr>
                <td>${t('postal_code')}</td>
                <td>${Router.plz || '-'}</td>
            ...
            ${form.crowd === 'retired' ? `
            <tr>
                <td>${t('monthly_pension')}</td>
                <td colspan="3">${formatCurrency(form.monthlyPensionAmount || 0)} CHF</td>
            </tr>
            ` : ''}
        </table>
    </div>
    ...
    <!-- 计算结果部分 -->
    <div class="section">
        <div class="section-title">${t('berechnungsergebnisse_heading') || 'Berechnungsergebnisse'}</div>
        
        <!-- IPV -->
        <div class="benefit-card ${ipvResult && !ipvResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('ipv_title')}</div>
                <div class="benefit-amount ${ipvResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(ipvResult?.annualBenefit || 0)} CHF/${t('annual_short') || 'Jahr'}
                </div>
            </div>
            <div>
                <strong>${t('monthly_short') || 'Monatlich'}:</strong> ${formatCurrency((ipvResult?.annualBenefit || 0) / 12)} CHF
        ...
        <div class="benefit-card ${elResult && !elResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('el_title')}</div>
                <div class="benefit-amount ${elResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(elResult?.annualBenefit || 0)} CHF/${t('annual_short') || 'Jahr'}
                </div>
            </div>
            <div>
                <strong>${t('monthly_short') || 'Monatlich'}:</strong> ${formatCurrency((elResult?.annualBenefit || 0) / 12)} CHF
        ...
        <div class="benefit-card ${faResult && !faResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('fa_title')}</div>
                <div class="benefit-amount ${faResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(faResult?.annualBenefit || 0)} CHF/${t('annual_short') || 'Jahr'}
                </div>
            </div>
            <div>
                <strong>${t('monthly_short') || 'Monatlich'}:</strong> ${formatCurrency((faResult?.annualBenefit || 0) / 12)} CHF
                ${faResult?.oneTime?.birth > 0 ? `<div>${t('birth_allowance')}: ${formatCurrency(faResult.oneTime.birth)} CHF (${t('onetime_payment')})</div>` : ''}
                ${faResult?.oneTime?.adoption > 0 ? `<div>${t('adoption_allowance')}: ${formatCurrency(faResult.oneTime.adoption)} CHF (${t('onetime_payment')})</div>` : ''}
        ...
        <div class="benefit-card ${shResult && !shResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('sozialhilfe_title')}</div>
                <div class="benefit-amount ${shResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(shResult?.annualBenefit || 0)} CHF/${t('annual_short') || 'Jahr'}
                </div>
            </div>
            <div>
                <strong>${t('monthly_short') || 'Monatlich'}:</strong> ${formatCurrency((shResult?.annualBenefit || 0) / 12)} CHF
    ...
    <!-- 免责声明 -->
    <div class="section">
        <div class="section-title">${t('disclaimer_important') || 'Wichtige Hinweise'}</div>
        <div class="note">
    ...
    <div class="footer">
        ${t('pdf_footer') || 'Erstellt mit Sozialleistungs-Rechner {year} | Diese Berechnung dient nur als Orientierungshilfe.'}
    </div>
</body>
</html>`;
}
...
async function generatePDF() {
  ...
  try {
    ...
    // 退休专属字段
    if (form.crowd === 'retired' && form.monthlyPensionAmount) {
      inputFields.push([t('monthly_pension') || 'Monatliche Rente', `${formatCurrency(form.monthlyPensionAmount)} CHF`]);
    }
...
/* 23. 模板函数 - 退休版专用：人群选择页面 */
const tmplCrowd = () => `
  <h2>${t('select_crowd_retired')}</h2>
  <p style="color:#666; margin-bottom:20px;">
    ${t('retired_mode_hint')}
  </p>
  <select id="sel-crowd">
    <option value="">${t('select_crowd_placeholder')}</option>
    <option value="retired" selected>${t('crowd_retired')} ${t('recommended_for_retired')}</option>
...
const tmplState = () => `
  ...
  <select id="sel-state">
    <option value="">${t('select_canton_placeholder')}</option>
    ${STATES.map(s => `<option value="${s}">${t(s + '_name') || s}</option>`).join('')}
  </select>
  <label>${t('postal_code')}</label>
  <input id="inp-plz" type="text" maxlength="4" placeholder="${t('postal_code_example')}" pattern="\\d{4}">
  <div class="button-group">
...
/* 输入表单 - 学生版专用：添加学生专属字段 */
const tmplForm = () => `
  <h2>${t('input_data_student')}</h2>
  
  <!-- 学生版专用提示 -->
  <div class="mode-notice">
    <strong>${t('student_mode_active')}</strong><br>
    ${t('student_hint')}
  </div>
  ...
  <form id="dynamic-form">
    ...
    <span class="hint">${t('income_hint')}</span>
    <input name="income" type="number" step="0.01" min="0" placeholder="${t('income_placeholder')}" required>

    <label>${t('assets')} <small>${t('assets_hint')}</small></label>
    <span class="hint">${t('assets_detail_hint')}</span>
    <input name="assets" type="number" step="0.01" min="0" placeholder="${t('assets_placeholder')}" required>

    <label>${t('health_insurance_premium')}</label>
    <span class="hint">${t('health_premium_hint')}</span>
    <input name="health_premium" type="number" step="0.01" min="0" placeholder="${t('health_premium_placeholder')}" required>

    <label>${t('monthly_rent')} (CHF)</label>
    <span class="hint">${t('rent_hint')}</span>
    <input name="monthlyRent" type="number" step="0.01" min="0" placeholder="${t('rent_placeholder')}" required>

    <!-- 成人固定为1（学生版通常单人） -->
    <label>${t('num_adults_single')}</label>
    <input name="numAdults" type="number" min="1" max="1" value="1" readonly style="background:#f0f8ff;">

    <!-- 新增：学生专属字段 -->
    <label>${t('is_fulltime_student')}</label>
    <span class="hint">${t('fulltime_student_hint')}</span>
    <div style="margin:5px 0 15px;">
      <label style="display:inline-block; margin-right:20px;">
        <input type="radio" name="isFulltimeStudent" value="yes" checked> ${t('yes_fulltime')}
      </label>
      <label style="display:inline-block;">
        <input type="radio" name="isFulltimeStudent" value="parttime"> ${t('parttime_label')}
      </label>
    </div>

    <label>${t('study_costs_monthly')}</label>
    <span class="hint">${t('study_costs_hint')}</span>
    <input name="studyCostsMonthly" type="number" step="0.01" min="0" placeholder="${t('study_costs_placeholder')}" value="0">

    <label>${t('has_stipendium')}</label>
    <span class="hint">${t('stipendium_hint')}</span>
    <div style="margin:5px 0 15px;">
      <label style="display:inline-block; margin-right:20px;">
    ...
    <div id="el-extra-fields" style="display:none">
      ...
      <div id="el-other-fields">
        ...
        <select name="nationality" required>
          <option value="">${t('select_nationality_placeholder')}</option>
          ...
          <option value="refugee_b">${t('nat_refugee')} ${t('refugee_b_status')}</option>
        </select>
        <label>${t('label_residence_years')}</label>
        <input name="residenceYears" type="number" min="0" max="100" placeholder="${t('residence_years_placeholder')}" required>
      </div>
    </div>
    ...
    <div id="sh-extra-fields" style="display:none; margin-top:20px; padding:15px; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px;">
      ...
      <span class="hint" style="color: #666; font-size: 0.9em;">${t('employment_status_hint_student') || 'Für Studierende oft \'other\' oder \'unemployed\''}</span>
      <select name="employmentStatus">
        <option value="" disabled selected>${t('select_option_placeholder')}</option>
      ...
      <input name="other_income_annual" type="number" step="0.01" min="0" placeholder="${t('other_income_placeholder')}" value="0">

      <label>${t('label_monthly_other_expenses')}</label>
      <input name="monthly_other_expenses" type="number" step="0.01" min="0" placeholder="${t('other_expenses_placeholder')}" value="0">

      <label style="color: #28a745;">✓ ${t('label_ipv_received_annual')}</label>
      <input name="ipvReceivedAnnual" type="number" step="0.01" min="0" placeholder="${t('ipv_received_placeholder')}" value="0" readonly style="background-color: #f0f8ff; border-color: #28a745;">

      <label style="color: #28a745;">✓ ${t('label_el_received_annual')}</label>
      <input name="elReceivedAnnual" type="number" step="0.01" min="0" placeholder="${t('el_received_placeholder')}" value="0" readonly style="background-color: #f0f8ff; border-color: #28a745;">
    </div>
  </form>
...
/* 输入表单 - 退休版专用：完整国际化修复 */
const tmplRetired = () => `
  <h2>${t('input_data_retired')}</h2>
 
  <!-- 退休版专用提示 -->
  <div class="mode-notice">
    <strong>${t('retired_mode_active')}</strong> ${t('retired_hint')}
  </div>
  
  <form id="dynamic-form">
    <!-- 基本财务信息 -->
    <label>${t('annual_income')} (CHF)</label>
    <span class="hint">${t('retired_income_hint') || 'Steuerbares Jahreseinkommen (ohne AHV/IV-Rente)'}</span>
    <input name="income" type="number" step="0.01" min="0" placeholder="${t('income_placeholder') || 'z.B. 30000'}" required>

    <label>${t('assets')} <small>${t('hint_assets_small') || '(steuerbares Reinvermögen)'}</small></label>
    <span class="hint">${t('hint_assets') || 'Steuerbares Reinvermögen des gesamten Haushalts'}</span>
    <input name="assets" type="number" step="0.01" min="0" placeholder="${t('assets_placeholder') || 'z.B. 100000'}" required>

    <label>${t('health_insurance_premium')}</label>
    <span class="hint">${t('hint_health_premium') || 'Jahresprämie der obligatorischen Grundversicherung'}</span>
    <input name="health_premium" type="number" step="0.01" min="0" placeholder="${t('health_premium_placeholder') || 'z.B. 3000'}" required>

    <label>${t('monthly_rent')} (CHF)</label>
    <span class="hint">${t('hint_monthly_rent') || 'Monatliche Miete oder Hypothekarzinsen'}</span>
    <input name="monthlyRent" type="number" step="0.01" min="0" placeholder="${t('rent_placeholder') || 'z.B. 1200'}" required>

    <label>${t('num_adults')}</label>
    <span class="hint">${t('hint_num_adults') || 'Anzahl Erwachsene im Haushalt'}</span>
    <input name="numAdults" type="number" min="1" max="10" value="1" required>

    <!-- 退休专属字段 - 养老金金额 -->
    <label>${t('monthly_pension')}</label>
    <span class="hint">${t('monthly_pension_hint') || 'Monatliche AHV- oder IV-Rente'}</span>
    <input name="monthlyPensionAmount" type="number" step="0.01" min="0" placeholder="${t('monthly_pension_placeholder') || 'z.B. 2000'}" required>
    ...
    <div id="el-extra-fields" style="display:block">
      ...
      <div id="el-other-fields">
        <label>${t('label_nationality')}</label>
        <select name="nationality" required>
          <option value="">${t('select_nationality_placeholder') || '-- Bitte wählen --'}</option>
          <option value="ch_eu">${t('nat_ch_eu')}</option>
          <option value="non_eu_eea">${t('nat_non_eu')}</option>
          <option value="refugee_f">${t('nat_refugee')} ${t('refugee_f_status')}</option>
        ...
        <label>${t('label_residence_years')}</label>
        <input name="residenceYears" type="number" min="0" max="100" placeholder="${t('residence_years_placeholder_retired') || 'z.B. 20'}" required>
      </div>
    </div>
    ...
    <div id="sh-extra-fields" style="display:none; margin-top:20px; padding:15px; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px;">
      ...
      <label>${t('label_employment_status')}</label>
      <span class="hint">${t('employment_status_hint_retired') || 'Für Rentner oft "unable" oder "other"'}</span>
      <select name="employmentStatus">
      ...
      <label>${t('label_other_income_annual')}</label>
      <input name="other_income_annual" type="number" step="0.01" min="0" placeholder="${t('other_income_placeholder_retired') || 'z.B. 0'}" value="0">

      <label>${t('label_monthly_other_expenses')}</label>
      <input name="monthly_other_expenses" type="number" step="0.01" min="0" placeholder="${t('other_expenses_placeholder_retired') || 'z.B. 0'}" value="0">

      <label style="color: #28a745;">✓ ${t('label_ipv_received_annual')}</label>
      <input name="ipvReceivedAnnual" type="number" step="0.01" min="0" placeholder="${t('ipv_received_placeholder') || '0.00'}" value="0" readonly style="background-color: #f0f8ff; border-color: #28a745;">

      <label style="color: #28a745;">✓ ${t('label_el_received_annual')}</label>
      <input name="elReceivedAnnual" type="number" step="0.01" min="0" placeholder="${t('el_received_placeholder') || '0.00'}" value="0" readonly style="background-color: #f0f8ff; border-color: #28a745;">
    </div>
  </form>
...
// ========== 模块初始化函数（供loader.js调用）==========
window.initCrowdModule = async function () {  // ← 添加 async
  console.log('Initializing retired crowd module (fullscreen mode)');

  try {
    // ========== 新增：加载退休版专用CSS ==========
    await FullscreenManager.loadCSS().catch(e => {
      console.warn('Retired CSS load failed, continue anyway', e);
    });
    ...
    if (app) {
      app.innerHTML = '';
      app.classList.add('retired-module-active');  // ← 新增
    }

    // 开始渲染（从州选择页面开始）
    render('state');

    console.log('Retired fullscreen module initialized successfully');
  } catch (error) {
    console.error('Retired module init failed:', error);
...
console.log('Retired module loaded, ready for initialization');
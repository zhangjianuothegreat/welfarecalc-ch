...
const BENEFIT_TYPE_MAP = {
  // 简写: 全称（用于键名拼接）
  'sh': 'sozialhilfe',     // sh 变成 sozialhilfe
  'ipv': 'ipv',            // ipv 保持不变
  'el': 'el',              // el 保持不变
  'fa': 'fa',              // fa 保持不变
  'alv': 'alv'             // alv 保持不变
...
/**
 * 获取标准化的福利类型名称
 * @param {string} type - 输入的福利类型
 * @returns {string} 标准化的全称
 */
function getNormalizedBenefitType(type) {
  ...
  // 反向查找：如果是全称，确保它存在于映射中
  const found = Object.entries(BENEFIT_TYPE_MAP).find(([short, full]) => full === type);
  if (found) {
    return type; // 已经是全称
  }

  // 未知类型，返回原值并输出警告
  console.warn(`[Benefit Type] 未知的福利类型: "${type}"，请检查是否需要添加到映射表`);
  return type;
}
 ...
 * @returns {string} 完整的翻译键名
 */
function getAuthorityKey(state, type) {
  const normalized = getNormalizedBenefitType(type);
  return `${state}_${normalized}_authority_key`;
}
 ...
 * @returns {string} 完整的翻译键名
 */
function getContactAddressKey(state, type) {
  const normalized = getNormalizedBenefitType(type);
  return `${state}_${normalized}_contact_address_key`;
}
// ========== 新增的初始化函数 ==========
/**
 * 初始化福利类型映射表并验证
 */
function initBenefitTypeMap() {
  console.log('[Benefit Type Map] 初始化福利类型映射表:');

  // 输出所有映射关系用于调试
  Object.entries(BENEFIT_TYPE_MAP).forEach(([short, full]) => {
    console.log(`  ${short} → ${full}`);

    // 验证反向映射
    const reverseFound = Object.entries(BENEFIT_TYPE_MAP).find(([s, f]) => f === full);
    if (!reverseFound) {
      console.warn(`[警告] 全称 "${full}" 没有对应的简写映射`);
    }
  });

  // 验证是否存在简写与全称相同但未映射的情况
  const allTypes = ['ipv', 'el', 'fa', 'sh', 'sozialhilfe', 'alv'];
  allTypes.forEach(type => {
    if (!BENEFIT_TYPE_MAP[type] && !Object.values(BENEFIT_TYPE_MAP).includes(type)) {
      console.warn(`[警告] 类型 "${type}" 未在映射表中定义`);
    }
  });

  console.log('[Benefit Type Map] 初始化完成');
}
// ========== 初始化函数结束 ==========
...
async function loadLanguage(lang = 'de') {
  try {
    ...
    window.LANG = {
      lang: 'de',
      lang_de: 'Deutsch',
      getApplicationKey_not_found: 'Application information not available',
      familienkasse: 'Familienkasse',
      not_specified: 'Not specified',
      application_info: 'Application Information',
      info: 'Info',
      adults_short: 'Adults',
      children_short: 'Children',
      young_adults_short: 'Young adults',
      health_insurance_premium_short: 'Health premium',
      save_hint: 'Save',
      save_instructions: 'Press Ctrl+P (Windows) or Cmd+P (Mac) and select "Save as PDF".',
      one_time: 'one-time',
      no_one_time_payments: 'No one-time payments',
      error_skipped: 'Calculation skipped.',
      error_not_applicable: 'Calculation not applicable.',
      application_info_load_failed: 'Application information could not be loaded. Please visit the official cantonal website.',
...
function render(step, isBack = false) {
  ...
  // ========== 全屏模式控制（对齐家庭版逻辑） ==========
  // 如果回到「州选择」页面，保持全屏（因为这是单人版的起点）
  if (step === 'state') {
    console.log('Rendering state step, ensuring fullscreen mode (single mode)');
    // 确保进入全屏模式（如果尚未进入）
    if (window.FullscreenManager && !document.body.classList.contains('module-fullscreen')) {
      window.FullscreenManager.enter();
      app.classList.add('single-module-active');
    }
    // 不需要恢复主页面元素，因为全屏模式下它们应该隐藏
  }
  // 如果进入 form 或 result 页面，确保全屏
  else if ((step === 'form' || step === 'result') &&
    window.FullscreenManager &&
    !document.body.classList.contains('module-fullscreen')) {
    console.log(`Rendering ${step} step, ensuring fullscreen (single mode)`);
    window.FullscreenManager.enter();
    app.classList.add('single-module-active');
  }
  // ========== 全屏控制结束 ==========
...
const FullscreenManager = {
  ...
  enter: function () {
    ...
    app.classList.add('fullscreen-mode', 'single-module-active');

    // 强制居中和宽度限制（与 CSS 变量配合）
  ...
  exit: function () {
    ...
    app.classList.remove('fullscreen-mode', 'single-module-active');
  ...
  reset: function () {
    ...
    const app = document.getElementById('app');
    if (app) {
      app.classList.remove('single-module-active', 'family-module-active', 'fullscreen-mode');
      app.innerHTML = '';
    }
  },

  // 【新增】动态加载 main_single.css（单人版专用）
  loadCSS: async function () {
    // 避免重复加载
    if (document.querySelector('link[href*="main_single.css"]')) {
      console.log('[FullscreenManager] main_single.css 已存在，跳过重复加载');
      return;
    }
    ...
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = 'css/main_single.css';  // 请确认你的文件路径是否正确

    // 可选：添加 onload 事件确认加载成功
    return new Promise((resolve, reject) => {
      link.onload = () => {
        console.log('[FullscreenManager] main_single.css 加载成功');
        resolve();
      };
      link.onerror = () => {
        console.error('[FullscreenManager] main_single.css 加载失败，路径：css/main_single.css');
  ...
  if (step === 'state') {
    ...
    if (exitBtn) {
      exitBtn.onclick = () => {
        ...
        const app = document.getElementById('app');
        if (app) {
          app.classList.remove('single-module-active');
          // 清空app容器并显示默认提示
          app.innerHTML = `
...
function collectForm() {
  ...
  // Single mode: adults fixed to 1, children and FA fields set to 0
  ...
  console.log('Form collected (单人版):', Router.form);
}
...
function fillResultPage() {
  ...
  fillFAOneTime(b.fa);
  // ✅ 关键修正：使用与家庭版完全一致的 showFormula 调用
...
function fillFAOneTime(fa) {
  ...
  if (fa.oneTime.birth > 0) {
    html += `<div><span class="onetime-badge">${t('one_time')}</span> ${t('birth_allowance')}: ${fa.oneTime.birth.toFixed(2)} CHF</div>`;
  }
  if (fa.oneTime.adoption > 0) {
    html += `<div><span class="onetime-badge">${t('one_time')}</span> ${t('adoption_allowance')}: ${fa.oneTime.adoption.toFixed(2)} CHF</div>`;
  }
  div.innerHTML = html;
}
/* 16. 显示用户输入信息（集成 FA 输入显示） */
/* 16. 显示用户输入信息（单人版专用：去除孩子相关行） */
...
async function showFormula(b, boxId) {
  ...
  if (boxId.includes('fa') && (!b || b.error)) {
    ...
    // 使用翻译键，如果不存在则显示硬编码文本
    const title = t('fa_not_applicable_single') || 'Familienzulagen nicht anwendbar';
    const message = 'Für alleinstehende Personen ohne Kinder oder Auszubildende besteht kein Anspruch auf Familienzulagen (FA).';

    errorHtml += `<strong>${title}</strong><br>`;
    errorHtml += message;
    errorHtml += '</div>';
  ...
  // 4. 根据benefitType选择正确的数据源（完全采用家庭版的隔离逻辑）
  if (benefitType === 'fa') {
    // FA: 只从FA_INFO获取
  ...
  if (rule) {
    ...
    // 提取法律信息（使用家庭版的extractLegalBasis函数）
    ...
    html += `<div class="note-hint"><strong>${t('hint') || 'Hinweis'}:</strong> ${t('application_info_load_failed') || 'Antragsinformationen konnten nicht geladen werden. Bitte besuchen Sie die offizielle Kantonsseite.'}</div>`;
  }
...
function getErrorMessage(errorObj) {
  ...
  if (typeof errorObj === 'string') {
    ...
    return t('err_general_no_entitlement');
  }
...
<body>
    ...
    <!-- 用户信息部分 -->
    <div class="section">
        <div class="section-title">${t('pdf_input_data_title') || 'Eingabedaten'}</div>
    ...
    <!-- 计算结果部分 -->
    <div class="section">
        <div class="section-title">${t('pdf_results_title') || 'Berechnungsergebnisse'}</div>
        
        <!-- IPV -->
        <div class="benefit-card ${ipvResult && !ipvResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('ipv_title') || 'Individuelle Prämienverbilligung (IPV)'}</div>
                <div class="benefit-amount ${ipvResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(ipvResult?.annualBenefit || 0)} ${t('chf_per_year') || 'CHF/Jahr'}
                </div>
            </div>
            <div>
        ...
        <div class="benefit-card ${elResult && !elResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('el_title') || 'Ergänzungsleistungen (EL)'}</div>
                <div class="benefit-amount ${elResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(elResult?.annualBenefit || 0)} ${t('chf_per_year') || 'CHF/Jahr'}
                </div>
            </div>
            <div>
        ...
        <div class="benefit-card ${faResult && !faResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('fa_title') || 'Familienzulagen (FA)'}</div>
                <div class="benefit-amount ${faResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(faResult?.annualBenefit || 0)} ${t('chf_per_year') || 'CHF/Jahr'}
                </div>
            </div>
            <div>
        ...
        <div class="benefit-card ${shResult && !shResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('sozialhilfe_title') || 'Sozialhilfe'}</div>
                <div class="benefit-amount ${shResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(shResult?.annualBenefit || 0)} ${t('chf_per_year') || 'CHF/Jahr'}
                </div>
            </div>
            <div>
    ...
    <!-- 免责声明 -->
    <div class="section">
        <div class="section-title">${t('important_notes_title') || 'Wichtige Hinweise'}</div>
        <div class="note">
            <strong>${t('disclaimer_content')}</strong><br>
            ${t('pdf_disclaimer_final_check') || 'Die endgültige Prüfung erfolgt durch die zuständigen Stellen.'} 
            ${t('pdf_disclaimer_submit') || 'Bitte reichen Sie die erforderlichen Unterlagen persönlich bei der zuständigen Behörde ein.'}
        </div>
    </div>

    <div class="footer">
        ${t('pdf_footer_created_with') || 'Erstellt mit Sozialleistungs-Rechner'} ${new Date().getFullYear()} | 
        ${t('pdf_footer_disclaimer') || 'Diese Berechnung dient nur als Orientierungshilfe.'}
    </div>
</body>
</html>`;
}
  ...
  try {
    ...
    const drawApplicationInfo = async (type, state) => {
      ...
      if (type === 'fa') {
        ...
        if (rule) {
          let authKey;
          if (rule.authority?.authority_key) {
            authKey = rule.authority.authority_key;
          } else {
            try {
              authKey = getApplicationKey(state, type, 'authority');
            } catch (e) {
              console.warn(`[PDF] Failed to get application key for ${type}:`, e);
              authKey = null;
            }
          }
          appInfo = {
  ...
  const htmlContent = `
    ...
    <body>
      <div class="header">
        ...
        <p><em>${t('pdf_offline_version') || 'Offline-Version - Speichern Sie diese Seite als PDF'}</em></p>
      </div>
      
      <div class="section">
        <h2>${t('pdf_input_data_title') || 'Eingabedaten'}</h2>
        <table>
          <tr><th>${t('pdf_table_col_field') || 'Feld'}</th><th>${t('pdf_table_col_value') || 'Wert'}</th></tr>
          ${Object.entries({
    [t('zielgruppe') || 'Personengruppe']: t('crowd_' + Router.crowd) || Router.crowd,
    [t('canton') || 'Kanton']: stateName,
    [t('postal_code') || 'Postleitzahl']: Router.plz || '-',
    [t('adults_short') || 'Erwachsene']: form.numAdults || 1,
    [t('children_short') || 'Kinder']: form.numChildren || 0,
    [t('young_adults_short') || 'Junge Erwachsene']: form.numEducation || 0,
    [t('annual_income') || 'Jahreseinkommen']: formatCurrency(form.income || 0) + ' CHF',
    [t('assets') || 'Vermögen']: formatCurrency(form.assets || 0) + ' CHF',
    [t('health_insurance_premium_short') || 'KK-Prämie']: formatCurrency(form.health_premium || 0) + ' CHF',
    [t('monthly_rent') || 'Monatsmiete']: formatCurrency(form.monthlyRent || 0) + ' CHF'
      ...
      <div class="section">
        <h2>${t('pdf_results_title') || 'Berechnungsergebnisse'}</h2>
        ${resultData.ipv ? `<div class="benefit-card">
          <h3>${t('ipv_short') || 'IPV'}: ${formatCurrency(resultData.ipv.annualBenefit || 0)} CHF/Jahr</h3>
        ...
        ${resultData.el && !resultData.el.error ? `<div class="benefit-card">
          <h3>${t('el_short') || 'EL'}: ${formatCurrency(resultData.el.annualBenefit || 0)} CHF/Jahr</h3>
          <p>${t('monthly_short') || 'Monatlich'}: ${formatCurrency((resultData.el.annualBenefit || 0) / 12)} CHF</p>
        </div>` : ''}
        
        ${resultData.fa ? `<div class="benefit-card">
          <h3>${t('fa_short') || 'FA'}: ${formatCurrency(resultData.fa.annualBenefit || 0)} CHF/Jahr</h3>
        ...
        ${resultData.sozialhilfe ? `<div class="benefit-card">
          <h3>${t('sh_short') || 'Sozialhilfe'}: ${formatCurrency(resultData.sozialhilfe.annualBenefit || 0)} CHF/Jahr</h3>
...
const tmplForm = () => `
  ...
  <div class="mode-notice">
    <strong>${t('single_mode_active') || 'Einzelpersonen-Modus aktiv'}</strong> – 
    ${t('single_mode_no_children') || 'keine Kinderfelder erforderlich'}. 
    ${t('fa_not_applicable') || 'Familienzulagen nicht anwendbar'}
  </div>
  <form id="dynamic-form">
    <label>${t('annual_income')} (CHF)</label>
    <span class="hint">${t('hint_annual_income') || 'Steuerbares Jahreseinkommen (aus der letzten definitiven Steuerveranlagung)'}</span>
    ...
    <span class="hint">${t('single_hint_health_premium') || 'Tatsächliche Jahresprämie der obligatorischen Grundversicherung'}</span>
...
window.initCrowdModule = async function () {
  console.log('Initializing SINGLE crowd module (fullscreen mode)');

  try {
    // 1. 加载单人版专用 CSS（如果有）
    await FullscreenManager.loadCSS().catch(e => {
      console.warn('Single CSS load failed, continue anyway', e);
    });
    ...
    if (app) {
      app.innerHTML = '';
      app.classList.add('single-module-active');  // 可选：加个单人版专用类
    }

    // 8. 直接从「州选择」页面开始（跳过人群选择）
    render('state');

    console.log('Single fullscreen module initialized successfully');
  } catch (error) {
    console.error('Single module init failed:', error);
    FullscreenManager.exit();  // 出错也要退出全屏
...
window.onload = async function () {
  console.log('Single module onload triggered');
  ...
  if (!isLoadedByLoader) {
    // 直接访问此文件时，直接初始化模块
    console.log('Direct access to single module, initializing directly');
    await window.initCrowdModule();
  } else {
    // 通过loader加载，等待loader调用initCrowdModule
    console.log('Single module loaded by loader, waiting for init...');
    // 什么都不做，等待loader调用initCrowdModule
  }
};

console.log('Single module loaded, ready for initialization');
/**
 * Kanton Graubünden (GR) – Sozialhilfe (SH) Algorithmus 2026
 * Rechtsgrundlage:
 * - Sozialhilfegesetz des Kantons Graubünden (SHG GR)
 * - SKOS-Richtlinien 2026
 *
 * Typ: Schätzmodell (nicht rechtsverbindlich)
 * Kompatibilität: main.js globaler Sozialhilfe-Router
 * Besonderheiten GR:
 * - Drei Mietzinsregionen (A/B/C) via PLZ → postal_data.json
 * - IPV wird vor SH abgezogen
 * - Integrationszulage (IZU) nur bei Erwerbstätigkeit
 * - Vereinfachte Kinder-/Ausbildungslogik mit GR-Korrektur
 */

export default function calculateSozialhilfe_GR(inputs, cantonRules) {
  ...
  /* --------------------------------------------------
   * 2. Grundbedarf (SKOS 2026 mit GR-Korrektur)
   * -------------------------------------------------- */
  ...
  grundbedarf +=
    additionalAdults * r.grundbedarf_monthly.per_additional_adult;

  // Vereinfachte Kinderlogik (Schätzmodell)
  const childrenUnder11 = Math.floor(numChildren / 2);
  const children11to15 = numChildren - childrenUnder11;

  grundbedarf +=
    childrenUnder11 * r.grundbedarf_monthly.per_child_under_11;
  grundbedarf +=
    children11to15 * r.grundbedarf_monthly.per_child_11_15;

  /**
   * GR-Spezial:
   * Junge Erwachsene in Ausbildung (19–25)
   * werden konservativ auf 80% des Erwachsenen-Grundbedarfs begrenzt
   * um Überbewertung zu vermeiden.
   */
  const educationFactor = 0.8;
  grundbedarf +=
    numEducation *
    (r.grundbedarf_monthly.per_child_over_15 * educationFactor);

  /* --------------------------------------------------
   * 3. Wohnkosten – Mietzinsmaxima A/B/C
   * -------------------------------------------------- */
  const rentInput = inputs.monthlyRent || 0;
  const rentSizeKey =
    totalPersons >= 5 ? '5p_plus' : `${totalPersons}p`;

  const region = inputs.region || 'region_b';
  const maxRent =
    r.rent_max_monthly?.[region]?.[rentSizeKey] ||
    r.rent_max_monthly?.region_b?.[rentSizeKey] ||
    0;

  const recognizedRent = Math.min(rentInput, maxRent);

  /* --------------------------------------------------
   * 4. Krankenkassenprämien – IPV zuerst
   * -------------------------------------------------- */
  ...
  const monthlyBenefit = Math.max(
    0,
    monthlyNeeds - (totalIncome + assetConsumptionMonthly)
  );
  ...
  return {
    ...
    explanation: {
      steps: [
        ...
        { label: 'step_health_premium_sh', value: healthPremiumNeeds.toFixed(2) },
        ...
        { label: 'step_asset_consumption_sh', value: assetConsumptionMonthly.toFixed(2) },
        {
          ...
          value: (totalIncome + assetConsumptionMonthly).toFixed(2)
        }
      ],
      note_key: 'GR_sh_calc_note'
    }
  };
}

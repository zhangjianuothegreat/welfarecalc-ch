/**
 * Kanton Zürich (ZH) – Sozialhilfe Berechnung 2026
 * Rechtsgrundlage: Sozialhilfegesetz des Kantons Zürich (SHG ZH) + SKOS-Richtlinien 2026
 * Typ: Schätzmodell (keine individuelle Behördenentscheidung)
 * Kompatibilität: main.js Sozialhilfe-Router
 * Besonderheiten ZH: Drei Mietzinsregionen (A: Stadt, B: Agglo, C: Rural) basierend auf PLZ, IPV-Offset, IZU nur bei Erwerbstätigkeit.
 */

export default function calculateSozialhilfe_ZH(inputs, cantonRules) {
  ...
  /* --------------------------------------------------
   * 1. Haushalt & Vermögen (SKOS 2026 mit ZH-Praxis)
   * -------------------------------------------------- */
  ...
  const userAssets = typeof inputs.taxableAssets === 'number'
  ...
  /* --------------------------------------------------
   * 3. Wohnkosten (ZH: Drei Regionen A/B/C basierend auf PLZ)
   * -------------------------------------------------- */
  ...
  const regionTable = r.rent_max_monthly[regionKey] ||
                      r.rent_max_monthly.region_b;

  const maxRent = regionTable[rentSizeKey] ||
                  regionTable['2p'] || 0;

  const recognizedRent = Math.min(rentInput, maxRent);

  /* --------------------------------------------------
   * 4. Krankenkassenprämien (IPV zuerst abziehen)
   * -------------------------------------------------- */
  ...
  /* --------------------------------------------------
   * 5. Integrationszulage (nur bei Erwerbstätigkeit)
   * -------------------------------------------------- */
  ...
  const remainingIpvMonthly = ipvMonthly > healthPremiumMonthly
    ? ipvMonthly - healthPremiumMonthly
    : 0;

  const otherBenefitsMonthly = remainingIpvMonthly +
  ...
  const assetConsumptionMonthly = excessAssets /

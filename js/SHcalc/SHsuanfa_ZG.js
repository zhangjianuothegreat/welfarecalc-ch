/**
 * Kanton Zug (ZG) – Sozialhilfe Berechnung 2026
 * Rechtsgrundlage: Sozialhilfegesetz des Kantons Zug (SHG ZG) + SKOS-Richtlinien 2026
 * Typ: Schätzmodell (keine individuelle Behördenentscheidung)
 * Kompatibilität: main.js Sozialhilfe-Router
 * Besonderheiten ZG: Hohe Lebenshaltungskosten, einheitliche (nicht regionale) Mietzinsmaxima, höhere Vermögensfreibeträge
 */

export default function calculateSozialhilfe_ZG(inputs, cantonRules) {
  ...
  const maxTotal = r.asset_freibetrag.max_total || 25000;
  ...
  /* --------------------------------------------------
   * 3. Wohnkosten (ZG: einheitliche hohe Mietzinsmaxima, keine Regionen)
   * -------------------------------------------------- */
  const rentInput = inputs.monthlyRent || 0;
  const rentSizeKey = totalPersons >= 5 ? '5p_plus' : `${totalPersons}p`;

  const maxRent = r.rent_max_monthly[rentSizeKey] ||
                  r.rent_max_monthly['2p'] || 0;

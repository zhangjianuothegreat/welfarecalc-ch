/**
 * Kanton Jura (JU) Sozialhilfe-Algorithmus 2026 - Auditor Final Version
 * Rechtsgrundlage: Loi sur l'aide sociale (LAS JU) + SKOS 2026
 * Nature: Estimation model (Schätzmodell), not individual decision
 * Compatibility: main.js global Sozialhilfe router
 * Revision: Fusion of Grok & GPT strengths - Enhanced comments/structure, detailed explanation, added max_total for accuracy.
 * Core: Strict regional rent caps (Urbain/Rural), child age simplification (with disclaimer), IPV offset for KK, conservative IZU (employed only).
 * Note: JU follows SKOS with French-speaking adjustments; rent maxima are cantonal averages with communal variations.
 */
export default function calculateSozialhilfe_JU(inputs, cantonRules) {
  const r = cantonRules?.sozialhilfe;
  if (!r || !r.grundbedarf_monthly) {
    return { error: 'err_missing_sh_rules', annualBenefit: 0 };
  }
  /* --------------------------------------------------
   * 1. Household & Assets (SKOS 2026 with JU practice)
   * -------------------------------------------------- */
  ...
  /* --------------------------------------------------
   * 3. Housing costs (JU: two regions - Urbain/Rural based on PLZ)
   * -------------------------------------------------- */
  const rentInput = inputs.monthlyRent || 0;
  const rentSizeKey = totalPersons >= 5 ? '5p_plus' : `${totalPersons}p`;
  const region = inputs.region || 'rural'; // fallback to rural (common in JU)
  const maxRent = r.rent_max_monthly[region][rentSizeKey] || r.rent_max_monthly.rural['2p'] || 0;
  ...
  /* --------------------------------------------------
   * 8. Asset consumption (SKOS standard divisor; no JU-specific slower rate)
   * -------------------------------------------------- */

...
const Router = {
  lang: 'de',
  crowd: null,  // 这个可以留着，但我们会自动设置为 'student'
  state: null,
  plz: null,
  form: {},
  rule: null,
  ...
  currentStep: 'state',  // 从 'crowd' 改为 'state'！
...
function resolvePath(path) {
  // 1. 如果已经是完整URL（http/https开头），直接返回
  if (path.startsWith('http://') || path.startsWith('https://')) {
    return path;
  }

  // 2. 如果是网站根路径（以/开头），拼接当前网站的origin（协议+域名+端口）
  if (path.startsWith('/')) {
    return window.location.origin + path;
  }

  // 3. 如果是相对路径（以./开头），基于当前页面URL解析
  if (path.startsWith('./')) {
    const baseUrl = window.location.href.split('/').slice(0, -1).join('/') + '/';
    return new URL(path, baseUrl).href;
  }

  // 4. 其他普通相对路径（无./前缀），添加./后再解析
  const baseUrl = window.location.href.split('/').slice(0, -1).join('/') + '/';
  return new URL(`./${path}`, baseUrl).href;
}
...
/**
 * 动态添加页面所需的所有CSS样式
 * 功能：创建style标签并注入预定义的CSS样式，然后添加到文档头部
 * 用途：统一管理页面样式，避免外部CSS文件依赖
 */
function addStyles() {
  // 创建style元素
  const style = document.createElement('style');

  // 定义CSS样式内容（使用模板字符串，保持良好的缩进和格式）
  style.textContent = `
/* 全局基础样式 */
body {
...
.benefit-details[open] .toggle-hint {
    content: "(${t('details_collapse')})"; /* 动态文字在JS里处理 */
}
...
/* 全屏容器样式 */
#app.module-fullscreen {
    position: fixed;
    top: 0;
    left: 0;
    width: 100vw;
    height: 100dvh; /* 使用动态视口高度，对移动端更友好 */
    z-index: 9999; /* 确保在最上层 */
    background: white;
...
/* 确保原有内容在模块内正确显示 */
#app.module-fullscreen .user-inputs-container,
#app.module-fullscreen .benefit-details {
    max-width: 600px;
    margin-left: auto;
    margin-right: auto;
}
...
@media (max-width: 600px) {
  ...
  console.log('Styles added and marked as loaded');
}
...
const FullscreenManager = {
  ...
  enter: function () {
    ...
    app.classList.add('fullscreen-mode', 'student-module-active');
  ...
  exit: function () {
    ...
    app.classList.remove('fullscreen-mode', 'student-module-active');
  ...
  reset: function () {
    ...
    const app = document.getElementById('app');
    if (app) {
      app.classList.remove('student-module-active', 'fullscreen-mode');
      app.innerHTML = '';
    }
  },

  // 动态加载 main_student.css（学生版专用）
  loadCSS: async function () {
    // 避免重复加载
    if (document.querySelector('link[href*="main_student.css"]')) {
      console.log('[FullscreenManager] main_student.css 已存在，跳过重复加载');
      return;
    }
    ...
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = 'css/main_student.css';  // 请确认你的文件路径是否正确

    return new Promise((resolve, reject) => {
      link.onload = () => {
        console.log('[FullscreenManager] main_student.css 加载成功');
        resolve();
      };
      link.onerror = () => {
        console.error('[FullscreenManager] main_student.css 加载失败');
  ...
  if (step === 'state') {
    ...
    if (exitBtn) {
      exitBtn.onclick = () => {
        ...
        const app = document.getElementById('app');
        if (app) {
          app.innerHTML = '<div style="text-align:center; padding:50px; color:#666;">请在上方选择您的人群类型</div>';
        }

        // 重置Router状态
        const currentLang = Router.lang;
        Router.crowd = 'student';
        Router.state = null;
        Router.plz = null;
        Router.form = {};
        Router.rule = null;
...
function collectForm() {
  ...
  // 确保人群类型是 student
  Router.form.crowd = 'student';  // 添加的这一行

  console.log('Form collected (学生版):', Router.form);
  // 额外日志：检查学生字段是否保存
  console.log('Student relevant inputs:', {
    isFulltimeStudent: Router.form.isFulltimeStudent,
    studyCostsMonthly: Router.form.studyCostsMonthly,
    hasStipendium: Router.form.hasStipendium
  });
}
...
async function runCalculation() {
  ...
  if (!Router.pendingSH) { // 第一次进来
    ...
    if (Router.resultData.el && !Router.resultData.el.error && (Router.resultData.el.annualBenefit || 0) > 0) {
      ...
      // 学生版额外调整：只在临时输入里加学生需求，不动原 form
      if (shInput.crowd === 'student') {
        const isFulltime = shInput.isFulltimeStudent === 'yes';
        // 从语言文件获取金额，或者使用数字但通过翻译键引用
        const studentExtraMonthly = isFulltime
          ? parseInt(t('student_extra_fulltime') || '300')
          : parseInt(t('student_extra_parttime') || '150');
        const studyCosts = shInput.studyCostsMonthly || 0;
        const totalStudentExtra = studentExtraMonthly + studyCosts;

        // 只加到临时对象的 monthly_other_expenses
        shInput.monthly_other_expenses = (shInput.monthly_other_expenses || 0) + totalStudentExtra;

        console.log(t('sh_student_hint') || 'Student extra added to SH temp input:', {
          isFulltime,
          studentExtraMonthly,
          studyCostsMonthly: studyCosts,
          totalAdded: totalStudentExtra,
          shInput_monthly_other_expenses: shInput.monthly_other_expenses
        });
      }
  ...
  // --- 流程 D: FA 计算（单人版专用：直接标记不适用） ---
  try {
    // 单人版永远没有孩子，所以 FA 不适用
    Router.resultData.fa = {
      error: 'not_applicable',
      message: t('fa_not_applicable_single') || 'Keine Kinder oder Auszubildende angegeben - Nicht anwendbar für Einzelpersonen',
      eligible: false,
      monthlyBenefit: 0,
      annualBenefit: 0,
      monthly: 0,
      ...
      breakdown: {
        children: { count: 0, monthlyTotal: 0 },
        education: { count: 0, monthlyTotal: 0 },
        special: { newborns: 0, adoptions: 0 }
      },
      explanation: { steps: [], note_key: 'fa_single_no_children' },
      meta: {
        state: Router.state,
        benefitType: 'fa',
        rulesApplied: []
    ...
    console.log('FA calculation skipped for single mode: not applicable');
  } catch (e) {
    console.error('FA calc error in single mode', e);
    Router.resultData.fa = {
...
function fillFAOneTime(fa) {
  ...
  if (!fa || !fa.oneTime || (fa.oneTime.birth === 0 && fa.oneTime.adoption === 0)) {
    div.innerHTML = `<span style="color:#666">${t('no_one_time_payments') || 'Keine Einmalzahlungen'}</span>`;
    return;
  }

  let html = '';
  if (fa.oneTime.birth > 0) {
    html += `<div><span class="onetime-badge">${t('onetime_payment') || 'Einmalig'}</span> ${t('birth_allowance')}: ${fa.oneTime.birth.toFixed(2)} CHF</div>`;
  }
  if (fa.oneTime.adoption > 0) {
    html += `<div><span class="onetime-badge">${t('onetime_payment') || 'Einmalig'}</span> ${t('adoption_allowance')}: ${fa.oneTime.adoption.toFixed(2)} CHF</div>`;
  }
  div.innerHTML = html;
}
...
function displayUserInputs() {
  ...
  html += `<tr><td>${t('health_insurance_premium_short') || 'KK-Prämie (Jahr)'}:</td><td>${formatCurrency(inputs.health_premium)} CHF</td></tr>`;
  html += `<tr><td>${t('monthly_rent')}:</td><td>${formatCurrency(inputs.monthlyRent)} CHF</td></tr>`;

  // 成人固定1
  html += `<tr><td>${t('num_adults')}:</td><td>1</td></tr>`;

  // 新增：学生专属信息
  html += `<tr><td>${t('is_fulltime_student') || 'Studienstatus'}:</td><td>${inputs.isFulltimeStudent === 'yes' ? t('yes_fulltime') || 'Vollzeit' : t('parttime') || 'Teilzeit'}</td></tr>`;
  html += `<tr><td>${t('study_costs_monthly') || 'Monatliche Studienkosten'}:</td><td>${formatCurrency(inputs.studyCostsMonthly)} CHF</td></tr>`;
  html += `<tr><td>${t('has_stipendium') || 'Stipendium'}:</td><td>${inputs.hasStipendium === 'yes' ? t('yes') : t('no')}</td></tr>`;

  // EL 相关（如果选择了）
  if (inputs.checkEL === 'yes') {
    if (inputs.isReceivingPension !== undefined) {
      let pensionText = inputs.isReceivingPension;
      if (inputs.isReceivingPension === 'ahv') pensionText = t('pension_type_ahv');
      else if (inputs.isReceivingPension === 'iv') pensionText = t('pension_type_iv');
      else if (inputs.isReceivingPension === 'no') pensionText = t('pension_type_none');
    ...
    if (inputs.nationality !== undefined && inputs.nationality !== '') {
      ...
      if (inputs.nationality === 'ch_eu') natText = t('nat_ch_eu');
      else if (inputs.nationality === 'non_eu_eea') natText = t('nat_non_eu');
      else if (inputs.nationality === 'refugee_f' || inputs.nationality === 'refugee_b') natText = t('nat_refugee');
...
function getErrorMessage(errorObj) {
  ...
  if (typeof errorObj === 'string') {
    ...
    if (errorObj.includes('skipped')) return t('error_skipped') || 'Berechnung wurde übersprungen.';
    if (errorObj.includes('calc_failed')) return t('error_calculation_failed');
    if (errorObj.includes('not_applicable')) return t('error_not_applicable') || 'Keine Berechnung erforderlich.';
    if (errorObj.includes('no_sh_when_el')) return t('no_sh_when_el_message');
...
async function showFormula(b, boxId) {
  ...
  if (b.explanation && Array.isArray(b.explanation.steps)) {
    ...
    b.explanation.steps.forEach(s => {
      // 使用翻译键，如果没有则回退到 s.label
      const labelKey = s.label_key || s.label;
      const labelText = t(labelKey) || s.label;
  ...
  if (rule) {
    ...
    // 官网（这里保留原有逻辑，和extractOfficialUrl形成双重保障）
    const officialUrlCard = benefitType === 'fa' ? rule.application?.url : (rule.application?.url || rule.official_url);
    if (officialUrlCard) {
      html += `<p><strong>Offizielle Webseite:</strong> <a href="${officialUrlCard}" target="_blank" style="color:#007bff;">${officialUrlCard}</a></p>`;
    }

    // 所需文件 - 根据福利类型使用不同的语言包键
    let docs = [];
    let langKey = '';

    if (benefitType === 'fa') {
      langKey = `${state}_fa_required_documents_list`;
    } else {
      langKey = `${state}_${benefitType}_required_documents_list`;
    }
    ...
    html += '</div>'; // 关闭application-card
    html += '</div>'; // 关闭legal-info-section
...
<body>
    <div class="header">
        ...
        <div class="subtitle">
            ${t('pdf_subtitle_for') || 'Berechnungsergebnisse für'} ${stateName || 'Schweiz'} (${state || 'CH'}) - ${t('pdf_created_on') || 'Erstellt am'} ${now}
        </div>
    </div>

    <!-- 用户信息部分 -->
    <div class="section">
        <div class="section-title">${t('pdf_input_data_title') || 'Ihre Eingabedaten'}</div>
        <table class="info-table">
            <tr>
                <th>${t('pdf_table_col_field') || 'Feld'}</th>
                <th>${t('pdf_table_col_value') || 'Wert'}</th>
                <th>${t('pdf_table_col_field') || 'Feld'}</th>
                <th>${t('pdf_table_col_value') || 'Wert'}</th>
            </tr>
            <tr>
                ...
                <td>${t('canton') || 'Kanton'}</td>
                <td>${stateName || t('not_specified') || 'Nicht angegeben'}</td>
            </tr>
            <tr>
                <td>${t('pdf_table_col_field') || 'Feld'}</td>
                <td>${t('pdf_table_col_value') || 'Wert'}</td>
            </tr>
            <tr>
                <td>${t('postal_code') || 'Postleitzahl'}</td>
                <td>${Router.plz || '-'}</td>
                <td>${t('number_adults') || 'Anzahl Erwachsene'}</td>
                <td>${form.numAdults || 1}</td>
            </tr>
            <tr>
                <td>${t('annual_income') || 'Jahreseinkommen'}</td>
                <td>${formatCurrency(form.income || 0)} CHF</td>
                <td>${t('number_children') || 'Anzahl Kinder'}</td>
                <td>${form.numChildren || 0}</td>
            </tr>
            <tr>
                <td>${t('assets') || 'Vermögen'}</td>
                <td>${formatCurrency(form.assets || 0)} CHF</td>
                <td>${t('young_adults_education') || 'Junge Erwachsene in Ausbildung'}</td>
                <td>${form.numEducation || 0}</td>
            </tr>
            <tr>
                <td>${t('health_insurance_premium') || 'Krankenkassenprämie'}</td>
                <td>${formatCurrency(form.health_premium || 0)} CHF</td>
                <td>${t('monthly_rent') || 'Monatsmiete'}</td>
                <td>${formatCurrency(form.monthlyRent || 0)} CHF</td>
            </tr>
            <!-- 新增：学生专属字段（只在学生模式显示） -->
            ${form.crowd === 'student' ? `
            <tr>
                <td>${t('is_fulltime_student') || 'Studienstatus'}</td>
                <td>${form.isFulltimeStudent === 'yes' ? t('yes_fulltime') || 'Vollzeit' : t('parttime') || 'Teilzeit'}</td>
                <td>${t('pdf_monthly_study_costs') || 'Monatliche Studienkosten'}</td>
                <td>${formatCurrency(form.studyCostsMonthly || 0)} CHF</td>
            </tr>
            <tr>
                <td>${t('has_stipendium') || 'Stipendium'}</td>
                <td colspan="3">${form.hasStipendium === 'yes' ? t('yes') : t('no')}</td>
            </tr>
            ` : ''}
        </table>
    </div>
    ...
    <!-- 计算结果部分 -->
    <div class="section">
        <div class="section-title">${t('pdf_calculation_results') || 'Berechnungsergebnisse'}</div>
        
        <!-- IPV -->
        <div class="benefit-card ${ipvResult && !ipvResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('ipv_name') || 'Individuelle Prämienverbilligung (IPV)'}</div>
                <div class="benefit-amount ${ipvResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(ipvResult?.annualBenefit || 0)} CHF/Jahr
                </div>
            </div>
            <div>
                <strong>${t('monthly') || 'Monatlich:'}</strong> ${formatCurrency((ipvResult?.annualBenefit || 0) / 12)} CHF
        ...
        <div class="benefit-card ${elResult && !elResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('el_name') || 'Ergänzungsleistungen (EL)'}</div>
                <div class="benefit-amount ${elResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(elResult?.annualBenefit || 0)} CHF/Jahr
                </div>
            </div>
            <div>
                <strong>${t('monthly') || 'Monatlich:'}</strong> ${formatCurrency((elResult?.annualBenefit || 0) / 12)} CHF
        ...
        <div class="benefit-card ${faResult && !faResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('fa_name') || 'Familienzulagen (FA)'}</div>
                <div class="benefit-amount ${faResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(faResult?.annualBenefit || 0)} CHF/Jahr
                </div>
            </div>
            <div>
                <strong>${t('monthly') || 'Monatlich:'}</strong> ${formatCurrency((faResult?.annualBenefit || 0) / 12)} CHF
                ${faResult?.oneTime?.birth > 0 ? `<div>${t('birth_allowance') || 'Geburtsszulage'}: ${formatCurrency(faResult.oneTime.birth)} CHF (${t('one_time') || 'einmalig'})</div>` : ''}
                ${faResult?.oneTime?.adoption > 0 ? `<div>${t('adoption_allowance') || 'Adoptionszulage'}: ${formatCurrency(faResult.oneTime.adoption)} CHF (${t('one_time') || 'einmalig'})</div>` : ''}
        ...
        <div class="benefit-card ${shResult && !shResult.error ? 'success' : 'warning'}">
            <div class="benefit-header">
                <div class="benefit-name">${t('sh_name') || 'Sozialhilfe'}</div>
                <div class="benefit-amount ${shResult?.annualBenefit ? '' : 'amount-zero'}">
                    ${formatCurrency(shResult?.annualBenefit || 0)} CHF/Jahr
                </div>
            </div>
            <div>
                <strong>${t('monthly') || 'Monatlich:'}</strong> ${formatCurrency((shResult?.annualBenefit || 0) / 12)} CHF
                ${shResult?.error ? `<div class="note">${getErrorMessage(shResult.error)}</div>` : ''}
                <!-- 学生版 SH 提示 -->
                ${shResult && !shResult.error && form.crowd === 'student' ? `
                <div style="margin-top:8px; font-size:10pt; color:#0066cc;">
                  <strong>${t('student_note') || 'Hinweis für Studierende:'}</strong> ${t('including_student_surcharge') || 'Inkl. Studentenzuschlag'} (${form.isFulltimeStudent === 'yes' ? '300' : '150'} CHF) + ${t('study_costs') || 'Studienkosten'} (${formatCurrency(form.studyCostsMonthly || 0)} CHF)
                </div>
                ` : ''}
            </div>
        </div>
    ...
    <!-- 免责声明 -->
    <div class="section">
        <div class="section-title">${t('pdf_important_notes') || 'Wichtige Hinweise'}</div>
        <div class="note">
            <strong>${t('pdf_disclaimer_heading') || 'Dies ist eine unverbindliche Vorab-Berechnung.'}</strong><br>
            ${t('disclaimer_content') || 'Die endgültige Prüfung erfolgt durch die zuständigen Stellen. Bitte reichen Sie die erforderlichen Unterlagen persönlich bei der zuständigen Behörde ein.'}
        </div>
    </div>
...
/* 20. PDF 生成 — 多语言字体兼容性完整解决方案 (最终修复版) - 从家庭版移植 */
async function generatePDF() {
  ...
  try {
    ...
    // 在 drawKeyValue 函数之后添加这个函数
    const drawKeyValueSingle = (label, value) => {
      pdf.setFontSize(10);
      pdf.setFont(FONT_FAMILY, FONT_BOLD);
      pdf.setTextColor(80, 80, 80);

      // 计算标签的换行
      const labelLines = pdf.splitTextToSize(processTextForPDF(label) + ':', 50);
      pdf.text(labelLines, margin + 5, yPos);

      // 计算值的换行
      pdf.setFont(FONT_FAMILY, FONT_NORMAL);
      pdf.setTextColor(0, 0, 0);
      const valueLines = pdf.splitTextToSize(processTextForPDF(value, true), contentWidth - 60);
      pdf.text(valueLines, margin + 55, yPos);

      // 根据最长行数调整 yPos
      yPos += Math.max(labelLines.length, valueLines.length) * LINE_HEIGHT + 4;
    };
    ...
    const drawBenefitCard = async (type, title, result) => {
      ...
      const hasAmount = annual > 0 && !hasError;
    ...
    // 学生专属字段（单行显示）
    if (form.crowd === 'student') {
      yPos += 2;
      const studentFields = [
        [t('is_fulltime_student') || 'Studieren Sie Vollzeit?',
        form.isFulltimeStudent === 'yes' ? (t('yes_fulltime') || 'Ja, Vollzeit') : (t('parttime') || 'Nein, Teilzeit')],
        [t('study_costs_monthly') || 'Monatliche Studienkosten',
        `${formatCurrency(form.studyCostsMonthly || 0)} CHF`],
        [t('has_stipendium') || 'Stipendium / Ausbildungsbeihilfe',
        form.hasStipendium === 'yes' ? (t('yes') || 'Ja') : (t('no') || 'Nein')]
      ];

      studentFields.forEach(([label, value]) => {
        drawKeyValueSingle(label, value);
      });
    }
  ...
  return `
    ...
    <div id="el-extra-fields" style="display:none">
      ...
      <div id="el-other-fields">
        ...
        <input name="residenceYears" type="number" min="0" max="100" placeholder="${t('placeholder_residence_years') || 'z.B. 12'}" required>
      </div>
    </div>
  `;
}

function renderSHFields() {
  return `
    <div id="sh-extra-fields" style="display:none; margin-top:20px; padding:15px; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px;">
      ...
      <span class="hint" style="color: #666; font-size: 0.9em;">${t('student_hint_employment_status') || 'Für Studierende oft "other" oder "unemployed"'}</span>
...
/* 23. 模板函数 - 学生版专用：人群选择页面 */
const tmplCrowd = () => `
  <h2>${t('select_crowd_student') || 'Willkommen beim Studenten-Rechner'}</h2>
  <p style="color:#666; margin-bottom:20px;">
    ${t('student_mode_hint') || 'Für Studierende (19–30 Jahre, in Ausbildung) – berechnet IPV, EL, SH und mögliche Stipendienzuschläge.'}
  </p>
  <select id="sel-crowd">
    <option value="">-- ${t('select_crowd')} --</option>
    <option value="student" selected>${t('crowd_student')} (empfohlen für Studierende)</option>
...
const tmplForm = () => `
  ...
  <div class="mode-notice">
    <strong>${t('student_mode_active') || 'Studenten-Modus aktiv'}</strong>
    <span class="mode-notice-text">${t('student_hint') || 'Bitte geben Sie Ihre Studien- und Lebenssituation ein. Dies beeinflusst mögliche Zuschläge bei Sozialhilfe (SH) und Prämienverbilligung (IPV).'}</span>
  </div>

  <form id="dynamic-form">
    <!-- 基本财务信息 -->
    <label>${t('annual_income')} (CHF)</label>
    <span class="hint">${t('hint_annual_income_student') || 'Steuerbares Jahreseinkommen (inkl. Nebenjobs, Stipendien etc.)'}</span>
    ...
    <span class="hint">${t('hint_monthly_rent_student') || 'Monatliche Miete (WG-Zimmer, Studentenwohnheim etc.)'}</span>
    ...
    <span class="hint">${t('hint_is_fulltime_student') || 'Vollzeit = mind. 20 ECTS pro Semester'}</span>
    <div style="margin:5px 0 15px;">
    ...
    <span class="hint">${t('hint_study_costs_monthly') || 'z.B. Semestergebühren, Bücher, Transport, Laptop etc. (nur nachweisbare Kosten)'}</span>
    ...
    <span class="hint">${t('hint_has_stipendium') || 'z.B. Kantonales Stipendium, Stiftungen, Sozialstipendium'}</span>
    <div style="margin:5px 0 15px;">
      <label style="display:inline-block; margin-right:20px;">
...
const tmplResult = () => `
  ...
  <div class="disclaimer-box">
    <strong>${t('disclaimer_important')}</strong>
...
// ========== 修改后的 initCrowdModule ==========
window.initCrowdModule = async function () {
  console.log('Initializing student crowd module (fullscreen mode)');
  ...
  try {
    ...
    if (app) {
      app.innerHTML = '';
      app.classList.add('student-module-active');
    }

    // 7. 加载学生版专用CSS（并等待完成）
    await FullscreenManager.loadCSS();
    console.log('Student CSS loaded successfully');
    ...
    console.log('Student fullscreen module initialized successfully');
  } catch (error) {
    console.error('Student module init failed:', error);
    FullscreenManager.exit();
...
function returnToHome() {
  ...
  const app = document.getElementById('app');
  if (app) {
    app.classList.remove('module-fullscreen');
  }
  ...
  // 可选：触发一个自定义事件，让主页知道我们返回了
  const event = new CustomEvent('moduleClosed', { detail: { module: 'student' } });
  document.dispatchEvent(event);

  console.log('Returned to home page');
}

function ensureTranslation(key, fallback) {
  const value = t(key);
  if (value === key && key !== fallback) {
...
function addBackToHomeButton() {
  ...
  backBtn.textContent = t('back_to_main') || 'Zurück zur Hauptseite';
  backBtn.onclick = returnToHome;

  document.body.appendChild(backBtn);
}

// ========== 修改 window.onload 函数（简化版）==========
window.onload = async function () {
  // 检查URL中是否有crowd参数（表示被loader加载）
  const urlParams = new URLSearchParams(window.location.search);
  const crowdParam = urlParams.get('crowd');

  if (crowdParam) {
    // 被loader加载，等待loader调用initCrowdModule
    console.log('Student module loaded by loader, waiting for init...');
  } else {
    // 独立启动，直接初始化
    console.log('Direct access to student module, initializing directly');
